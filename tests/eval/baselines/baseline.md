=== Archon Search Eval Report ===
generated_at: 2026-06-01T00:00:00.000000+00:00
corpus_root:  /Users/manczg/Documents/development/archon-search/tests/eval
queries:      33 (routing_disabled=0, routing_bypassed=0)
documents:    57

Quality metrics:
  recall_at_1        = 0.8704
  recall_at_3        = 0.9444
  recall_at_5        = 0.9815
  mrr                = 1.0000
  ndcg_at_5          = 0.9775
  ndcg_at_10         = 0.9809
  reranker_lift      = 0.0213
  routing_accuracy   = 0.9697
  routing_mrr_centroid     = 0.7500
  routing_p@1_centroid     = 0.6667
  routing_mrr_hybrid       = 0.7500
  routing_p@1_hybrid       = 0.6667

Latency (ms):
  latency_p50_ms     = n/a
  latency_p95_ms     = n/a

Note: latency was measured using deterministic eval backends (EvalEmbedderBackend, EvalRerankerBackend); values are not comparable to production runtime latency.

Rationale: refreshed for C1 Task 11.1 — alt-model collection added (2 documents, 2 retrieval queries). Floors reflect the current codebase after C1 pipeline IoC refactor (embedder-as-param). Routing metrics improved due to alt-model collection providing high centroid contrast.
