...
def test_sequential_failure_then_success_on_same_app() -> None:
    ...
    # Failure path + success path both enqueue telemetry
    assert writer_mock.enqueue.call_count == 2
    error_entry = writer_mock.enqueue.call_args_list[0].args[0]
    assert error_entry.latency_ms > 0
...
# ---------------------------------------------------------------------------
# Test 8: healthy search enqueues one success telemetry entry
# ---------------------------------------------------------------------------
