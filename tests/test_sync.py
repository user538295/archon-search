...
class TestSearchCollectionSync:
    ...
    async def test_sync_resolves_deep_collision_with_hash_fallback(self, tmp_path):
        ...
        with patch("archon_search.sync.path_to_collection_name", return_value="sessions"):
            syncer = SearchCollectionSync(pipeline)
    ...
    async def test_sync_handles_keyerror_on_drop_phantom_manifest_entry(self, tmp_path):
        ...
        syncer = SearchCollectionSync(pipeline)
        with patch("archon_search.sync.logger") as mock_logger:
            result = await syncer.sync([])
...
class TestManifestLookupByPath:
    def test_returns_none_when_no_manifest(self, tmp_path):
        from archon_search.sync import manifest_lookup_by_path
...
class TestTask46:
    ...
    async def test_sync_apply_changes_updates_collection_meta(self, tmp_path):
        """After _apply_collection_changes with needs_recompute=True, recompute_collection_meta is called."""
        ...
        # B5: recompute fires when signal raised (needs_recompute=True)
        from archon_search.collection_meta import CollectionMeta
...
class TestSyncCollectionMethod:
    ...
    async def test_sync_collection_collection_not_in_state_uses_defaults(self, tmp_path):
        ...
        from archon_search.progress import IndexingState, IndexingStateStore
        from archon_search.sync import SearchCollectionSync
...
# ===========================================================================
# B5 Task 6.2 — sync hot-path removal and checkpoint wiring
# ===========================================================================
...
def _make_pipeline_for_sync_centroid(tmp_path, meta=None, incremental_enabled=True, threshold=10_000):
    ...
    pipeline.ingest_file = AsyncMock(
        return_value=MagicMock(status="ok", needs_recompute=False)
    )
    pipeline.store.get_collection_meta = AsyncMock(return_value=meta)
    return pipeline
...
@pytest.mark.asyncio
async def test_sync_does_not_call_recompute_below_threshold(tmp_path) -> None:
    """Incremental sync below mutation threshold does not trigger recompute_collection_meta."""
    from archon_search.collection_meta import CollectionMeta
    from archon_search.sync import SearchCollectionSync

    col_dir = tmp_path / "myproject"
    col_dir.mkdir()
    (col_dir / "a.md").write_text("content a")
    (col_dir / "b.md").write_text("content b")
    (col_dir / "c.md").write_text("content c")

    meta = CollectionMeta(name="myproject", needs_recompute=False, mutations_since_recompute=3)
    pipeline = _make_pipeline_for_sync_centroid(tmp_path, meta=meta, threshold=10_000)

    # Mark as DONE so sync goes through incremental path
    state_store = _make_done_state(
        tmp_path, "myproject",
...
@pytest.mark.asyncio
async def test_sync_calls_recompute_when_signal_raised(tmp_path) -> None:
    """Incremental sync calls recompute_collection_meta when needs_recompute is True."""
    from archon_search.collection_meta import CollectionMeta
    from archon_search.sync import SearchCollectionSync
    ...
    meta = CollectionMeta(name="myproject", needs_recompute=True, mutations_since_recompute=0)
    pipeline = _make_pipeline_for_sync_centroid(tmp_path, meta=meta, threshold=10_000)
...
@pytest.mark.asyncio
async def test_sync_incremental_path_no_full_scan(tmp_path) -> None:
    """Incremental sync below threshold: store.get_all_vectors is never called."""
    from archon_search.collection_meta import CollectionMeta
    from archon_search.sync import SearchCollectionSync
    ...
    meta = CollectionMeta(name="myproject", needs_recompute=False, mutations_since_recompute=0)
    pipeline = _make_pipeline_for_sync_centroid(tmp_path, meta=meta, threshold=10_000)
    pipeline.store.get_all_vectors = AsyncMock(return_value=[])
    ...
    pipeline.store.count_documents.assert_not_called()


@pytest.mark.asyncio
async def test_sync_recompute_on_delete_threshold_crossing(tmp_path) -> None:
    """Post-loop check fires when mutations_since_recompute >= threshold even with needs_recompute=False."""
    from archon_search.collection_meta import CollectionMeta
    from archon_search.sync import SearchCollectionSync

    col_dir = tmp_path / "myproject"
    col_dir.mkdir()
    (col_dir / "kept.md").write_text("stays")

    meta = CollectionMeta(
        name="myproject", needs_recompute=False, mutations_since_recompute=5
    )
...
# ---------------------------------------------------------------------------
# Task 9.1: per-collection embedding model read-side fix
# ---------------------------------------------------------------------------


def test_sync_no_spurious_reindex_for_non_default_model(tmp_path):
    """Collection with active_embedding_model='model-X' (different from global) but
    indexed_embedding_model='model-X' (already correct) → no reindex triggered."""
    from archon_search.collection_meta import CollectionMeta
    from archon_search.sync import SearchCollectionSync
...
def test_sync_uses_collection_meta_over_state_store(tmp_path):
    """CollectionMeta.active_embedding_model is authoritative: even if global model matches
    indexed_embedding_model, if the collection's active model differs, reindex is triggered."""
    from archon_search.collection_meta import CollectionMeta
    from archon_search.sync import SearchCollectionSync
    ...
    new_f, changed_f, deleted = syncer._check_collection_changes(
        ...
        indexed_embedding_model="global-m",  # matches global, NOT meta.active
        indexed_chunk_size=512,
        meta=meta,
    )

    # Meta is authoritative: effective_model="model-X" != indexed="global-m" → reindex
...
def test_sync_meta_none_falls_back_to_global(tmp_path):
    ...
    assert new_f == [] and changed_f == [] and deleted == []


def test_sync_model_change_triggers_reindex(tmp_path):
    """When meta.active_embedding_model changed from indexed model, reindex is triggered."""
    from archon_search.collection_meta import CollectionMeta
    from archon_search.sync import SearchCollectionSync
    ...
    new_f, changed_f, deleted = syncer._check_collection_changes(
        ...
        source_path,
        file_mtimes={real_key: real_mtime},
        indexed_embedding_model="model-old",
        indexed_chunk_size=512,
        meta=meta,
    )
    # force_full_reindex → all eligible files returned as changed
    assert len(changed_f) > 0


def test_no_self_embedding_model_in_sync():
    """After rename, 'self._embedding_model' must not appear in sync.py."""
