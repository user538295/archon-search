...
- spaCy model resolution (dead code pending BE-15, still imported by
  model_validation.py): installed package → data-dir path → latched degrade
...
#
# `resolve_spacy_model`/`find_spacy_model`/`SpacyModelResolution` and their
# version-compatibility helpers are dead code w.r.t. GraphExtractor.extract()
# since BE-11 rewired it onto the prose extraction backend, AND (cycle-2) are
# no longer consumed by model_validation.py's graph_ner_status either — that
# probe now checks gliner import-ability directly instead. They remain in
# graph_extractor.py only for the install wizard's own spaCy-provisioning
# flow (tests/test_install_spacy_model.py), pending full deletion in BE-15
# ("the resolver chain"). These tests exercise the functions directly and are
# unaffected by either rewire.
# ---------------------------------------------------------------------------
...
def _place_data_dir_model(
    ...
    if spacy_version_spec is not None:
        (model_dir / "meta.json").write_text(
            json.dumps({"version": version, "spacy_version": spacy_version_spec})
        )
    return model_dir
...
# ---------------------------------------------------------------------------
# Version/specifier primitives (C2-T-9)
# ---------------------------------------------------------------------------
...
def _write_package_model(root: Path, version: str, spec: str) -> Path:
    ...
    (pkg / "meta.json").write_text(json.dumps({"version": version, "spacy_version": spec}))
    return pkg
