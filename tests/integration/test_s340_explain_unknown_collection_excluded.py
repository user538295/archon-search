...
Actual behaviour: ``SearchPipeline.explain`` raises ``CollectionNotFoundError`` as soon
as ANY requested name is absent from the namespace metadata
(``archon_search/pipeline.py`` — ``if missing: raise CollectionNotFoundError(missing)``),
and ``routes_explain.py`` maps that to ``404 {"detail": "collection not found"}``. One
unknown name therefore discards the results of every valid collection in the fan-out.
...
def test_explain_unknown_collection_is_excluded_not_fatal(
    ...
    """One valid + one unknown collection → 200, unknown reported in excluded_collections."""
    ...
    with make_real_app(tmp_path, monkeypatch) as (client, _cfg, api_key):
        ...
        data = resp.json()
        excluded_names = [e["name"] for e in data["excluded_collections"]]
        assert _UNKNOWN_COLLECTION in excluded_names, (
            f"unknown collection {_UNKNOWN_COLLECTION!r} missing from "
            f"excluded_collections {excluded_names}"
        )
