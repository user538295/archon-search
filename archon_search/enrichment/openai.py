...
- Raises on any transport failure (connection error, timeout, non-2xx
  ...
  (CommunityBuilder) catch and substitute None.
...
class OpenAIEnrichmentClient:
    ...
    def _extract_content(data: dict[str, Any]) -> str | None:
        ...
        if not content.strip():
            ...
            if finish_reason:
                _logger.warning(
                    "OpenAIEnrichmentClient: reply had no usable content "
                ...
                _logger.warning(
                    "OpenAIEnrichmentClient: reply had no usable content; treating "
