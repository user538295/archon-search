...
async def ingest(body: IngestRequest, request: Request) -> JobResponse | JSONResponse:
    ...
    # rather than a failed background job (A5c). The lock is released immediately
    # after the check so the background task self-locks via ingest_chunks — this
    # avoids threading _locked_by_caller=True through the entire pipeline.
    result = await acquire_collection_lock_or_503(search_store, body.collection)
    if isinstance(result, JSONResponse):
        return result
    if result is not None:
        result.release()  # release immediately; background task self-locks
