...
- `smoke` — excluded from default runs the same way as `live_benchmark` (`norecursedirs` primary, `-m` addopts filter secondary); every smoke test file sets `pytestmark = pytest.mark.xdist_group("smoke_e2e")` at module level to serialize onto one worker and prevent concurrent server subprocess instances; run separately with the command above. The `tests/smoke/docker/` subdirectory (Docker CLI smoke tests, DCS feature) is covered by the same `norecursedirs = ["tests/smoke"]` exclusion — no `pyproject.toml` change is needed; run with `uv run pytest tests/smoke/docker/ --no-cov`.
...
**ONE TEST SUITE AT A TIME: Never launch `uv run pytest` in a fire-and-forget background call (i.e. `run_in_background` without Monitor). Exception: subagents may use `run_in_background: true` + `Monitor` to outlast the ~120 s Bash foreground ceiling — Monitor actively watches the process so nothing is abandoned. Never start a new suite run while a previous one may still be alive. Before any test run, verify with `ps -Ao args= | grep -E '[/]bin/pytest|[u]v run pytest'` that no pytest workers are alive (`pgrep -fl pytest` self-matches the shell and must not be used; match on `args`, never on `comm` — `comm` truncates a Homebrew interpreter to `/opt/homebrew/Ce`, so the older `awk '$1 ~ /[Pp]ython/'` form reported "no workers" while a full suite was running, which is exactly how a second suite gets stacked onto a live one). Stacked suite runs multiply parallel workers and OOM-crashed the 48 GB machine on 2026-07-05. While iterating, run scoped paths (`uv run pytest tests/test_x.py --no-cov`); run the full suite once (via blocking call or background+Monitor) at task completion. Always report the wall-clock duration of every full suite run (last measured: ~163 s with `--no-cov -n 8`, 8433 tests, 2026-08-12).**
...
- `platform/` + `install/` — OS service install/uninstall. `install/` is a package (split from the former 2857-line `install.py`): `installer.py` (the `InstallerProtocol`/`BaseInstaller`/`DryRunInstaller`/`RealInstaller` strategy hub + `create_installer` factory — the sole public symbol, imported by `cli/install_cmd.py`), plus leaf modules `errors.py`, `lock.py`, `config_writer.py` (owns `WizardFeatures`), `prewarm.py`, `render.py`, `licenses.py`, `wizard.py`, `extras.py`, `service_ops.py`. `install/__init__.py` re-exports the full prior surface, so `from archon_search.install import X` is unchanged; tests patch at the real call-site module (`install.installer.*`, `install.prewarm.*`, etc.), not the package. Secrets: both the Linux unit and the macOS `run-server.sh` launchd wrapper source `~/.archon-search/.secrets.env` (created 0600 by the wizard when HyDE or RAG Fusion is enabled; absent file is a no-op).

### Server (`archon_search/server/`)

`app.py` builds the FastAPI app; routes are split per resource (`routes_*.py`); `schemas.py` + `schemas_telemetry.py` hold the Pydantic models. All endpoints except `GET /health` and `GET /ready` require a `Bearer` token. **Lifespan startup never awaits slow work** — uvicorn binds the listening socket only after lifespan startup returns, so anything awaited there keeps the port closed (clients get `ConnectError`, not a served 503). Both the eager model warm-up (`app.state._warmup_task`) and the startup `collection_sync.sync()` over all configured collections (`app.state._startup_sync_task`, spawned only when `pinned_collections + collections` is non-empty) are spawned with `asyncio.create_task` and tracked in `app.state._background_tasks`; both are best-effort (failure logs WARNING). `app.state.warmup_result` (`None` → `"pending"` → `"done"`/`"failed"`; `"pending"` is set *before* the task starts so there is no ready-looking gap; also surfaced verbatim as `warmup_result` on `GET /status`) is the warm-up progress signal `GET /ready` gates on: with `[database].eager_load_embedders = true` and `warmup_result == "pending"`, `/ready` answers 503 with `checks.models = "pending"`; a **terminal** `warmup_result == "failed"` is checked next (before the separate `model_validation` result) and reads `checks.models = "fail"` without gating `ready` — mirrors `checks.sync = "fail"` below. `GET /ready`'s `checks` gains a third field, `sync` (`"pending"|"ok"|"fail"`): `"pending"` — and a 503 — while the startup sync task is still running, unconditionally regardless of `eager_load_embedders`; `"fail"` when the startup sync either raised an exception or completed with per-collection errors in its `SyncResult.errors` (both set `app.state._startup_sync_failed`) — this does NOT gate `ready` (a corrupted collection must not wedge the pod forever), so `/ready` answers 200 with `checks.sync: "fail"`; `"ok"` otherwise. The same `app.state.sync_lock` the task holds while running makes `POST /sync` answer 409 for that whole window. The D6 `model_validation` result stays informational and never gates `ready`. Both `warmup_result` and its sync-side counterpart `sync_result` (`StartupSyncResult`, same `"pending"`/`"done"`/`"failed"` shape; `null` when no collections are configured) are surfaced verbatim on `GET /status`. A single `@app.exception_handler(EmbedderNotReadyError)` in `create_app` maps a wedged/still-loading embedder-cache load to a sanitized `503` + `Retry-After: 30` (`{detail: EMBEDDER_NOT_READY_DETAIL, code: "embedder_not_ready"}`, never `str(exc)`) across every plain HTTP route not already handling it locally (`routes_search.py` keeps its own copy to also emit stage timings and a telemetry entry); the mounted `/mcp` sub-app is NOT covered (separate middleware stack) — its 5 embedder-resolving tools and the 4 background job-task sites (`routes_jobs.py` ×3, `routes_export.py`) each catch it individually and leave the job `FAILED` with the same sanitized message. `POST /search` additionally bounds its own `embedder_cache.get_or_load()` wait to `_EMBEDDER_LOAD_WAIT_TIMEOUT_SECONDS = 30s` (`routes_search.py`), separate from the cache's own `_LOAD_WAIT_TIMEOUT_SECONDS = 120s` dedup-wait bound (`embedder_cache.py`) — a cold-start search now fails fast at 30s instead of holding the connection for up to 120s. `GET /openapi.json` is the authoritative API contract — keep it in sync, and record breaking changes in `BREAKING.md`. **CSP120** adds two new endpoints: `POST /sync` (`routes_sync.py` — triggers `SearchCollectionSync.sync()` as a `SyncJob`) and `POST /collections/{name}/reindex-metadata` (`routes_collections.py` — runs `SearchStore.reindex_metadata()` as a `MetadataReindexJob`). Both follow the `rebuild_communities` route pattern (QUEUED→RUNNING before returning 202).
...
Connect-failure wording: every user-facing `_CONNECT_FAIL` handler (submission paths *and* the `--wait` poll loop) calls `_server_connect_fail_msg(base_url)` — two mid-poll handlers in `maintenance_cmd.py` catch `_CONNECT_FAIL` without calling it (one continues past a transient failure, one re-raises) — which probes `GET {base_url}/ready` — a `503` carrying `checks.storage == "ok"` and either `checks.models == "pending"` or `checks.sync == "pending"` prints the "starting up" hint, anything else prints "not running" *provided the probe itself was usable*; an unusable probe (connect failure, non-JSON body) falls through to the launchd/systemd `_get_service().status()` check instead, which can still print the starting hint. Because the probe hits the server process itself, the hint works for a foreground `archon-search serve` too.

**CSP120**: all write commands are HTTP proxies — they submit jobs to the running server and return a job ID immediately. The following commands require `archon-search serve` to be running and accept `--api-url` / `--api-key`: `collection add` (proxies `POST /collections/`), `collection remove` (proxies `DELETE /collections/{name}`), `collection reindex` (proxies `POST /collections/{name}/reindex`), `collection reindex-metadata` (proxies `POST /collections/{name}/reindex-metadata`), `ingest` (proxies `POST /ingest`), `sync` (proxies `POST /sync`). All except `remove` support `--wait` (polls `GET /jobs/{id}` via `_poll_job` in `_helpers.py`). `jobs status <job_id>` (`jobs_cmd.py`) is a new one-shot status-check command. `collection list` (proxies `GET /collections/`) and `collection info` (proxies `GET /collections/{name}`) are also server proxies and require the server to be running. Two new server endpoints were added: `POST /sync` (`routes_sync.py`) and `POST /collections/{name}/reindex-metadata` (`routes_collections.py`).
...
(see above), increment its `N` and update the date to actual date. Entries without `(×N)` (pre-migration) count
...
**Size cap — 30 lines, non-negotiable:**
`learnings.md` must always stay **under 30 lines**. If it is already at or
above 30 lines when you open it, compact before adding anything. After every
update review the whole file and re-compact so it stays under the cap:
- Keep the length of the lines unter 256 chars.
...
- Only evict when the file would still exceed 30 lines after compression — never proactively. When eviction is needed, remove lowest `N` first; break ties by oldest date. Add new entries after eviction, not before.
If an update would push the file past 30 lines, compress first, then add.
