...
Connect-failure wording: every `_CONNECT_FAIL` handler (submission paths *and* the `--wait` poll loop) calls `_server_connect_fail_msg(base_url)`, which probes `GET {base_url}/ready` — a `503` carrying `checks.models == "pending"` prints the "starting up" hint, anything else prints "not running". Because the probe hits the server process itself, the hint works for a foreground `archon-search serve` too; the launchd/systemd `_get_service().status()` probe is only the no-`base_url` fallback.
