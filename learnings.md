...
- **[2026-09-07] (×32) test vacuity**: split `or` asserts; overrides start OPPOSITE. Every ABSENCE assert needs a PRESENCE anchor + the deleted wording. A scenario saying "persisted" = read back via GraphStore, not `result.*`. A tester e2e adds a LAYER.
...
- **[2026-09-07] (×7) 3rd-party wiring**: verify params per INPUT FORMAT AND per CHECKPOINT, on the REAL model: GLiNER `relations` no-op off-RelEx; bare `"other"` ate all spans; `label<>desc`→0 relations; but descriptive `related_to` LIFTS typed yield.

## Plan-Making & Agent Process
- **[2026-09-07] (×175) verify claims/state**: grep-verify every cite; prove repros vs unmodified HEAD (`git worktree add --detach`). Diffing WIP vs HEAD reads own work as drift. Distrust a reviewer's "untested X" — often covered by a sibling test.
- **[2026-09-07] (×51) subagents**: final text is DISCARDED; a mid-run API death leaves NOTHING. Nested agents (clean-code-review's fan-out, plain reviewers) can leak results to the GRANDPARENT and return nothing — run the skill inline yourself.
- **[2026-09-07] (×11) guards**: allowlist entries must STILL match else empty; new tests IMPORT `_ENGINE_PATTERN`; `eval/backends.py`→`eval_hash` drift. Deleted allowlisted file → `git rm` + strike BOTH guards. Unspecified `xfail` cannot fail.
