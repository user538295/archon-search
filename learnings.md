...
- **[2026-09-05] (×168) verify claims/state**: grep-verify every cite; prove repros vs unmodified HEAD (`git worktree add --detach`). Diffing WIP vs HEAD reads own work as drift. Distrust a reviewer's "untested X" — often covered by a sibling test.
- **[2026-09-05] (×47) subagents**: final text is DISCARDED; a mid-run API death leaves NOTHING. A nested fan-out skill (clean-code-review) in a subagent never synthesises — its group agents outlive the wrapper, leaking STATUS to the GRANDPARENT.
- **[2026-09-05] (×3) full-suite/guards**: self-tightening allowlist asserts each entry STILL matches→shrinks to empty. Editing `eval/backends.py` (even a comment) drifts `compute_eval_hash`→refresh baseline.json `eval_hash` only; latency is machine-specific.
