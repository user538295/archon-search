...
- **[2026-09-06] (×31) test vacuity**: split `or` asserts; overrides start OPPOSITE. Every ABSENCE assert needs a PRESENCE anchor, and pins the deleted wording, not a generic substring. A tester e2e adds a LAYER; grep the dev's tests first.
...
- **[2026-09-06] (×174) verify claims/state**: grep-verify every cite; prove repros vs unmodified HEAD (`git worktree add --detach`). Diffing WIP vs HEAD reads own work as drift. Distrust a reviewer's "untested X" — often covered by a sibling test.
- **[2026-09-06] (×50) subagents**: final text is DISCARDED; a mid-run API death leaves NOTHING. Nested agents (clean-code-review's fan-out, plain reviewers) can leak results to the GRANDPARENT and return nothing — run the skill inline yourself.
- **[2026-09-06] (×8) full-suite/guards**: a self-tightening allowlist (each entry STILL matches) shrinks to empty. Any `eval/backends.py` edit drifts `compute_eval_hash`→refresh `eval_hash`. A NEW test must IMPORT `_ENGINE_PATTERN`, never restate it.
