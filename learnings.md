...
- **[2026-09-06] (×28) test vacuity**: split `or` asserts; overrides start at the OPPOSITE value. Assert an MB figure per LINE (`18 MB` ⊂ `1218 MB`). A tester e2e must add a LAYER (CLI transcript) — grep the dev task's own tests first or you restate them.
...
- **[2026-09-06] (×29) install/wizard**: doc advertising an unprinted option → fix the CODE. A pinned prompt admits ONE wording. `is_multilingual=False` re-resolves `prof`. CliRunner echoes `input()` prompts into `result.output` (real emission order).
...
- **[2026-09-06] (×172) verify claims/state**: grep-verify every cite; prove repros vs unmodified HEAD (`git worktree add --detach`). Diffing WIP vs HEAD reads own work as drift. Distrust a reviewer's "untested X" — often covered by a sibling test.
- **[2026-09-06] (×49) subagents**: final text is DISCARDED; a mid-run API death leaves NOTHING. Nested agents (clean-code-review's fan-out, plain reviewers) can leak results to the GRANDPARENT and return nothing — run the skill inline yourself.
- **[2026-09-06] (×6) full-suite/guards**: a self-tightening allowlist (each entry STILL matches) shrinks to empty. Any `eval/backends.py` edit drifts `compute_eval_hash`→refresh baseline `eval_hash`. A removed-engine literal in a NEW test trips S42.
