...
## What Has Worked
- **[2026-08-12] (×53) bug briefs**: write the failing repro FIRST; probe every config permutation. A green test proves only code+test agree — diff it against the DOC contract. Fix at the guard layer, never a call-site proxy.
- **[2026-08-12] (×53) brief triage**: a re-filed brief whose filename is already in `Completed/` = regression or a second failure mode. A doc-only fix is never a behavioral fix — read the code after closing a DOC-tagged report.
...
- **[2026-08-14] (×13) CLI HTTP-proxy**: grep `tests/` for patches of removed symbols first; mock `httpx.*` where USED; `ConnectError` before `HTTPError`; keys must be 64-hex. A non-terminal `.get(f, sentinel)` in a `while` loop is a silent hang.
...
- **[2026-08-15] (×19) install/wizard**: when a doc advertises a shipped-and-accepted option the code never prints, fix the CODE. Base-URL probes must prefix `http://`, splitting authority from path before injecting the port. Rest: the archive.
...
- **[2026-08-18] (×73) verify before acting**: `git ls-files Documentation/Completed/ | grep <ID>` + `git log --oneline -- <file the fix touched>` for LATER commits, then read CURRENT source — a "fix pre-existing failures" commit can revert a fix.
...
- **[2026-08-18] (×26) doc close-out**: grep the WHOLE tree (incl. `README.md`, `*.toml.example`) for the old invariant string — per-file scope orphans siblings. Never put a `>` block between table rows. Order: api-ref→catalog→CLAUDE.md→manual.
