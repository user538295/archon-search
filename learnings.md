...
- **[2026-09-07] (×10) guards**: allowlist entries must STILL match else empty; new tests IMPORT `_ENGINE_PATTERN`; `eval/backends.py`→`eval_hash` drift. Deleted allowlisted file → `git rm` + strike BOTH guards. Unspecified `xfail` cannot fail.
