...
- **[2026-09-07] (×33) test vacuity**: split `or` asserts; overrides start OPPOSITE. Every ABSENCE assert needs a PRESENCE anchor + the deleted wording. A scenario saying "persisted" = read back via GraphStore, not `result.*`. A tester e2e adds a LAYER.
