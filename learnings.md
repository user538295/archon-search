...
- **[2026-09-06] (×30) test vacuity**: split `or` asserts; overrides start OPPOSITE. Every ABSENCE assert needs a PRESENCE anchor, and pins the deleted wording, not a generic substring. A tester e2e adds a LAYER; grep the dev's tests first.
...
- **[2026-09-06] (×7) full-suite/guards**: a self-tightening allowlist (each entry STILL matches) shrinks to empty. Any `eval/backends.py` edit drifts `compute_eval_hash`→refresh `eval_hash`. A NEW test must IMPORT `_ENGINE_PATTERN`, never restate it.
