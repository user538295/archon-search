...
- **[2026-09-06] (×26) test vacuity**: split `or` asserts; overrides start at the OPPOSITE value. A guard tripping the FIRST filter never reaches the branch it names. A shared `MagicMock.read` ignoring its `n` arg silently voids a bounded `read(size+1)`.
...
- **[2026-09-06] (×28) install/wizard**: doc advertising an unprinted option → fix the CODE. A pinned prompt admits ONE wording. `is_multilingual=False` must re-resolve `prof`. `resolve_torch_device(coreml)`→`mps` in the dev venv, not cpu.
...
- **[2026-09-06] (×171) verify claims/state**: grep-verify every cite; prove repros vs unmodified HEAD (`git worktree add --detach`). Diffing WIP vs HEAD reads own work as drift. Distrust a reviewer's "untested X" — often covered by a sibling test.
