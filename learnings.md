...
- **[2026-09-05] (×23) test vacuity**: split `or` asserts; overrides start at the OPPOSITE value. A guard tripping the FIRST filter never reaches the branch it names — feed a value that clears the filter but is spared only by the target branch.
...
- **[2026-09-05] (×167) verify claims/state**: grep-verify every cite; prove repros vs unmodified HEAD (`git worktree add --detach`). Diffing WIP vs HEAD reads own work as drift. Distrust a reviewer's "untested X" — often covered by a sibling test.
- **[2026-09-05] (×46) subagents**: final text is DISCARDED; a mid-run API death leaves NOTHING. A nested fan-out skill (clean-code-review) in a subagent never synthesises — its group agents outlive the wrapper, leaking STATUS to the GRANDPARENT.
- **[2026-09-05] (×2) full-suite/guards**: stale `tests/eval/corpus/**/*.pyc`→`load_eval_corpus` fails, trash first. A `git ls-files` guard reads the INDEX; `trash`ing a tracked test leaves it listed→FileNotFoundError; `git rm` it (stages).
- **[2026-08-27] (×36) doc close-out**: grep the WHOLE tree (incl. `README.md`, `*.toml.example`) for the old invariant string — per-file scope orphans siblings. Never put a `>` block between table rows. Order: api-ref→catalog→CLAUDE.md→manual.
