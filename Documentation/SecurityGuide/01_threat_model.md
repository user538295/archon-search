...
| Process → bearer auth | `APIKeyMiddleware.dispatch` in `archon_search/server/middleware_auth.py:25`. A validated namespace is stamped onto `request.state.namespace`. The middleware exempts four paths: `/health`, `/docs`, `/openapi.json`, `/redoc` (`middleware_auth.py:16`). | Bearer-validated HTTP requests (and the four exempt paths). |
...
- **Multi-tenant SaaS isolation.** The server is single-process, single-key by default. The `namespaces` map provides separation between *cooperating* clients, not against malicious tenants. Tracked under `SEC-1` in [`../Architecture/530_technical_debt_refactoring_roadmap.md`](../Architecture/530_technical_debt_refactoring_roadmap.md) and as roadmap item D7.
