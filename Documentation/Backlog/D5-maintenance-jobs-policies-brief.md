...
All questions resolved 2026-06-21.

- **Q1 — Orphan cleanup on watcher-managed collections**: **DECIDED: run on all collections.** Use `_collection_locks` to avoid concurrent conflict. Log at DEBUG when a watcher-managed collection is scanned so operators can detect watcher issues. Rationale: watcher is forward-only; server-downtime orphans would never be cleaned under a skip policy.
- **Q2 — Stale-collection detection**: **DECIDED: expose `mutations_since_recompute` + `centroid_recompute_threshold` raw in `collection_health`; no `stale` bool, no `stale_threshold_days` config.** Rationale: committing to a `stale` boolean requires defining "stale" before D5.1; raw counters are non-breaking and give D5.1 a direct upgrade path.
- **Q3 — Retry count tracking**: **DECIDED: `.maintenance-state.json` keyed by source file path.** Rationale: only approach that survives job eviction (7-day window) and correctly enforces per-file cross-session bound. Auto-reset the counter when a job transitions to `SUCCEEDED`.
- **Q4 — Integrity check scope**: **DECIDED: surface `meta_chunk_count` + `actual_chunk_count` raw in `collection_health`; no `integrity_ok` bool.** Rationale: the O(1) check is worth doing now, but the boolean locks in "integrity" semantics before D5.1 defines them. D5.1 adds `integrity_ok` as a derived field.
- **Q5 — `retry_max_attempts` semantics**: **DECIDED: per-file total across sessions**, tracked in `.maintenance-state.json` keyed by source path. Auto-reset on `SUCCEEDED` to restore retry eligibility after a fix. Rationale: only option that prevents indefinite retry of permanently broken files across restarts.
