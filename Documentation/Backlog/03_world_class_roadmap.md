...
- [ ] **E1. GraphRAG / entity-relationship retrieval (item 33)** — three-stage implementation gated by eval harness. (1) Entity extraction at ingest: optional `archon-search[graph]` extra; configurable LLM (Haiku by default) extracts entities (people, concepts, systems, events, code symbols) and typed relationships (USES, IMPLEMENTS, CONTRADICTS, DEPENDS_ON, AUTHORED_BY, etc.) from each chunk; stored in per-collection `_graph_nodes` and `_graph_edges` LanceDB tables; each node gets a stable `entity_id: str` (SHA-256 hex of `entity_name.strip().lower()`) to enable cross-collection resolution (E8). Graph store: Kuzu embedded graph DB for collections > 10k edges; NetworkX in-memory for smaller collections. (2) Three retrieval variants via `graph_mode=local|global|naive` on `/search` and MCP `search`: `naive` — entity name expansion only, find chunks mentioning the same named entities as the query (lowest cost); `local` — Leiden community detection clusters semantically related entities; retrieve community summary chunks + member chunks (best for focused questions); `global` — traverse the entire knowledge graph for high-level synthesis questions like "What are the main architectural patterns?" (expensive, full graph scan). (3) Graph-path provenance in `/explain`: every graph-retrieved chunk shows its traversal path `(query_entity -> relationship -> neighbour_entity -> chunk)`. Citation expansion: graph edges enable context expansion beyond adjacent chunk_ids — if entity A DEPENDS_ON entity B, the graph retriever pulls chunks about B even if they score low on vector/FTS. Config: `[graph].enabled`, `[graph].extraction_model`, `[graph].max_community_size`, `[graph].leiden_resolution`. Gated by eval harness: `graph_mrr >= baseline_mrr`.
- [ ] **E2. TTL, scoping, and entity graph** — (a) TTL: every chunk/fact gains optional `expires_at: datetime|null`. Background maintenance (D5 extension) prunes expired chunks and writes a `"delete"` mutation log entry. TTL set at ingest via chunk metadata `ttl_seconds` or collection default `CollectionMeta.default_ttl_seconds`. REST: `GET /collections/{name}/expiring?within_hours={n}` shows chunks expiring soon. (b) Scoping: every chunk gains optional `scopes: list[str]` (free-form prefix-matched: `user:alice`, `agent:codex-session-1`, `app:project-X`, `run:task-123`). Search accepts `scope_filter=` to restrict results. (c) Entity graph: Requires E1 active for entity data (reads E1's `_archon_graph_{col}_nodes`/`_archon_graph_{col}_edges` LanceDB tables). E1 entity extraction must generate a stable content-hashed `entity_id` (via `make_stable_entity_id(entity_type, entity_name)`) at extraction time to enable cross-collection resolution. `GET /graph/{collection}` returns the entity graph as adjacency JSON — node list (entity name, entity type, chunk count, salience) and edge list (entity_a, relationship, entity_b, weight, source_chunk_ids). Cross-collection entity resolution: normalised entity names are content-hashed to a stable `entity_id`; `GET /graph/cross-collection?collections=a,b` returns a merged graph. Minimum acceptance: TTL pruning tested with an expired chunk; scope filter tested with two non-overlapping scopes; entity graph endpoint tested with a seeded graph.
- [ ] **E2c. Graph salience upgrade — TF-IDF entity scoring** — upgrade `GraphNode.salience` from per-collection chunk frequency (`distinct_chunks / total_chunks`) to a TF-IDF-style score that weights down entities common across all collections: `salience = (chunk_freq_in_collection / total_chunks_in_collection) * log(total_collections / collections_containing_entity + 1)`. Requires a cross-collection entity frequency table (or a global scan over all `_archon_graph_*_nodes` tables at extraction time) to compute the IDF denominator. Prerequisite: E2b (which ships chunk-frequency salience as the baseline). Re-extraction required on upgrade. Minimum acceptance: salience scores for domain-specific entities rank higher than salience scores for ubiquitous entities across a multi-collection test fixture.
- [ ] **E3. Streaming search results (item 27)** — SSE endpoint `POST /search/stream` (and MCP `search_stream` tool) that emits each scored chunk as a `data:` event as soon as the reranker scores it, rather than waiting for the full cross-encoder pass. Event schema: `{chunk_id, score, text, metadata, stage}`. Final event: `{done: true, total_returned, total_candidates, timing_ms}`. Backpressure: server drops unsent candidates if client disconnects. Client consumes a `text/event-stream` response; first chunk arrives after reranker scores candidate 1, not after all N candidates. Auth: same Bearer token. Config: `[search].stream_batch_size` (how many candidates to score before flushing; default 1). Reduces perceived latency for large rerank sets (top_k=50+) from full-pass wait to first-result latency.
- [ ] **E4. Python SDK (item 28a)** — superseded by G11 (full spec with openapi-python-client, PyPI package, SDK contract tests); see Phase G.
- [ ] **E5. TypeScript SDK (item 28b)** — superseded by G12 (full spec with hey-api/openapi-ts, npm package, dual ESM/CJS); see Phase G.
- [ ] **E6. Per-collection access-control policies (item 30)** — builds on item 5 and D7.
- [ ] **E7. Connector and federation architecture (item 15)** — pluggable source connectors with sync checkpoints, ACL propagation, source-specific change detection.
- [ ] **E8. Admin / debug UI (item 29)** — local single-page web app served at `GET /ui` (static files bundled into the package; no external CDN). Panels: (1) Collections browser — list collections, doc count, salience distribution, tier breakdown, last-indexed timestamp, ACL status; (2) Search playground — run `/search` or `/explain` from the UI, see ranked results with citation snippets, RRF scores, routing path, per-stage timings; (3) Jobs queue — live job list with status, progress, warnings, retry history; cancel button per job; (4) Telemetry viewer — recent search entries (no raw queries), truncated_count, result distribution, latency percentiles; (5) Keys manager — list active keys, revoke, rotate; (6) Maintenance status — last run timestamp, policy outcomes, salience floor stats. Auth: same Bearer token via `?token=` query param or `Authorization` header. Implementation: HTMX + minimal CSS over existing REST endpoints — no frontend build step, no npm, no bundler.
...
Worth tracking, not the first moves. Items below have since been promoted to the roadmap (E1 streaming → Phase E; E6 admin UI → Phase E; G9 OpenAI shim, G13 citation panel, G17 ANN index quantization, G18 multi-tenancy → Phase G above) — this section now covers only items with no current placement:
...
```mermaid
quadrantChart
    title Remaining items — effort vs. impact
    x-axis Low effort --> High effort
    y-axis Low impact --> High impact
    quadrant-1 Plan deliberately
    quadrant-2 Pay down soon
    ...
    "E2 Python SDK (→G11)": [0.4, 0.48]
    "E3 TypeScript SDK (→G12)": [0.5, 0.48]
    "E4 Per-collection ACL": [0.4, 0.57]
    "E5 Connectors": [0.85, 0.65]
    "E6 Admin UI": [0.75, 0.4]
    "E7 GraphRAG": [0.82, 0.72]
    "E8 TTL+scoping+entity graph": [0.65, 0.7]
...
**Defer until triggered** (high effort, currently low impact): E6 admin UI, F3 multimodal, F4 horizontal scale, F5 pluggable backends.
