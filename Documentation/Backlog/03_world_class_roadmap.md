...
- [ ] **F3. GraphRAG / entity-relationship retrieval (item 33)** — three-stage implementation gated by eval harness. (1) Entity extraction at ingest: optional `archon-search[graph]` extra; configurable LLM (Haiku by default) extracts entities (people, concepts, systems, events, code symbols) and typed relationships (USES, IMPLEMENTS, CONTRADICTS, DEPENDS_ON, AUTHORED_BY, etc.) from each chunk; stored in per-collection `_graph_nodes` and `_graph_edges` LanceDB tables. Graph store: Kuzu embedded graph DB for collections > 10k edges; NetworkX in-memory for smaller collections. (2) Three retrieval variants via `graph_mode=local|global|naive` on `/search` and MCP `search`: `naive` — entity name expansion only, find chunks mentioning the same named entities as the query (lowest cost); `local` — Leiden community detection clusters semantically related entities; retrieve community summary chunks + member chunks (best for focused questions); `global` — traverse the entire knowledge graph for high-level synthesis questions like "What are the main architectural patterns?" (expensive, full graph scan). (3) Graph-path provenance in `/explain`: every graph-retrieved chunk shows its traversal path `(query_entity -> relationship -> neighbour_entity -> chunk)`. Citation expansion: graph edges enable context expansion beyond adjacent chunk_ids — if entity A DEPENDS_ON entity B, the graph retriever pulls chunks about B even if they score low on vector/FTS. Config: `[graph].enabled`, `[graph].extraction_model`, `[graph].max_community_size`, `[graph].leiden_resolution`. Gated by eval harness: `graph_mrr >= baseline_mrr`.
...
### Cluster 4 — Explainability and human UX (beats R2R, docmost, agentmemory, Kotaemon)
...
- [ ] **G18. Binary quantization for large collections** — two-stage retrieval for collections > `[database].bq_min_chunks` (default 500k). (1) Coarse pass: every chunk gets a `vec_binary: bytes` column storing the INT1/bit-packed float embedding (threshold-at-zero: positive → 1, negative → 0; packed as `ceil(dim/8)` bytes). LanceDB Hamming distance scan retrieves top-K coarse candidates (~32× faster than float32 ANN). (2) Exact re-rank: float32 embedding loaded only for the coarse top-K; final score is true cosine similarity. Enabled via `[database].binary_quantization: bool` (default false); automatically skipped for collections below the threshold. Eval harness regression-gates recall@10 with quantization enabled vs disabled before allowing it as a collection default. Minimum acceptance: `vec_binary` column populated at ingest; two-stage search tested end-to-end; recall regression test added to eval suite.

- [ ] **G19. Multi-tenancy at storage level** — beyond the current namespace map + ACL filtering (which share a single LanceDB on disk). Storage-level isolation: each namespace gets its own LanceDB directory under `get_data_dir() / "tenants" / {namespace} /`; `SearchStore` instantiated per-namespace; collections isolated within their namespace's LanceDB instance. Migration: `POST /admin/migrate-to-tenants` migrates all existing collections to namespace-partitioned directories using the D2 export/import flow with zero downtime. Shared resources: embedding model cache and reranker remain shared (read-only). Auth: namespace derived from `KeyRecord.namespace`; cross-namespace access rejected at middleware layer. REST: `GET /namespaces` admin endpoint lists all tenants with storage usage and doc count. Config: `[storage].multi_tenant: bool` (default false). Minimum acceptance: two namespaces confirmed unable to read each other's collections even with a valid token for one; storage path isolation verified; migration tested with a pre-seeded single-namespace store.
...
Worth tracking, not the first moves. Items below have since been promoted to Phase G (see G10, G14, G18, G19, E1, E6 above) — this section now covers only items with no current placement:
...
quadrantChart
    ...
    "G18 Binary quantization": [0.55, 0.5]
...
51. ⬜ G1. LLM-extracted fact collections.
52. ⬜ G2. Memory mutation history and conflict resolution.
53. ⬜ G3. TTL, scoping, and entity graph.
...
68. ⬜ G18. Binary quantization for large collections.
