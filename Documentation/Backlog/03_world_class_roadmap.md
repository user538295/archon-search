...
- [ ] **F3. GraphRAG / entity-relationship retrieval (item 33)** — three-stage implementation gated by eval harness. (1) Entity extraction at ingest: optional `archon-search[graph]` extra; configurable LLM (Haiku by default) extracts entities (people, concepts, systems, events, code symbols) and typed relationships (USES, IMPLEMENTS, CONTRADICTS, DEPENDS_ON, AUTHORED_BY, etc.) from each chunk; stored in per-collection `_graph_nodes` and `_graph_edges` LanceDB tables; each node gets a stable `entity_id: str` (SHA-256 hex of `entity_name.strip().lower()`) to enable cross-collection resolution (G3). Graph store: Kuzu embedded graph DB for collections > 10k edges; NetworkX in-memory for smaller collections. (2) Three retrieval variants via `graph_mode=local|global|naive` on `/search` and MCP `search`: `naive` — entity name expansion only, find chunks mentioning the same named entities as the query (lowest cost); `local` — Leiden community detection clusters semantically related entities; retrieve community summary chunks + member chunks (best for focused questions); `global` — traverse the entire knowledge graph for high-level synthesis questions like "What are the main architectural patterns?" (expensive, full graph scan). (3) Graph-path provenance in `/explain`: every graph-retrieved chunk shows its traversal path `(query_entity -> relationship -> neighbour_entity -> chunk)`. Citation expansion: graph edges enable context expansion beyond adjacent chunk_ids — if entity A DEPENDS_ON entity B, the graph retriever pulls chunks about B even if they score low on vector/FTS. Config: `[graph].enabled`, `[graph].extraction_model`, `[graph].max_community_size`, `[graph].leiden_resolution`. Gated by eval harness: `graph_mrr >= baseline_mrr`. *(Moved from Phase F: F3 requires only A1, A4, and B6 — all shipped. No dependency on F1 salience or F2 memory tiers. Prioritised early because graph retrieval closes the single largest architectural gap vs R2R, Kotaemon, and agentmemory.)*
- [ ] **G3. TTL, scoping, and entity graph** — (a) TTL: every chunk/fact gains optional `expires_at: datetime|null`. Background maintenance (D5 extension) prunes expired chunks and writes a `"delete"` mutation log entry. TTL set at ingest via chunk metadata `ttl_seconds` or collection default `CollectionMeta.default_ttl_seconds`. REST: `GET /collections/{name}/expiring?within_hours={n}` shows chunks expiring soon. (b) Scoping: every chunk gains optional `scopes: list[str]` (free-form prefix-matched: `user:alice`, `agent:codex-session-1`, `app:project-X`, `run:task-123`). Search accepts `scope_filter=` to restrict results. (c) Entity graph: Requires F3 active for entity data (reads F3's `_graph_nodes`/`_graph_edges` LanceDB tables). F3 entity extraction must generate a stable content-hashed `entity_id` (SHA-256 of normalised entity name) at extraction time to enable cross-collection resolution. `GET /graph/{collection}` returns the entity graph as adjacency JSON — node list (entity name, entity type, chunk count, salience) and edge list (entity_a, relationship, entity_b, weight, source_chunk_ids). Cross-collection entity resolution: normalised entity names are content-hashed to a stable `entity_id`; `GET /graph/cross-collection?collections=a,b` returns a merged graph. Minimum acceptance: TTL pruning tested with an expired chunk; scope filter tested with two non-overlapping scopes; entity graph endpoint tested with a seeded graph. *(Moved from Phase G Cluster 1: entity graph sub-item requires F3; TTL and scoping sub-items have no prior-phase dependencies. All three land here so `GET /graph/{collection}` ships in the same release as graph extraction.)*
...
- [ ] **E1. Streaming search results (item 27)** — SSE endpoint `POST /search/stream` (and MCP `search_stream` tool) that emits each scored chunk as a `data:` event as soon as the reranker scores it, rather than waiting for the full cross-encoder pass. Event schema: `{chunk_id, score, text, metadata, stage}`. Final event: `{done: true, total_returned, total_candidates, timing_ms}`. Backpressure: server drops unsent candidates if client disconnects. Client consumes a `text/event-stream` response; first chunk arrives after reranker scores candidate 1, not after all N candidates. Auth: same Bearer token. Config: `[search].stream_batch_size` (how many candidates to score before flushing; default 1). Reduces perceived latency for large rerank sets (top_k=50+) from full-pass wait to first-result latency.
- [ ] **E2. Python SDK (item 28a)** — superseded by G12 (full spec with openapi-python-client, PyPI package, SDK contract tests); see Phase G.
- [ ] **E3. TypeScript SDK (item 28b)** — superseded by G13 (full spec with hey-api/openapi-ts, npm package, dual ESM/CJS); see Phase G.
...
Worth tracking, not the first moves. Items below have since been promoted to the roadmap (E1 streaming → Phase E; E6 admin UI → Phase E; G10 OpenAI shim, G14 citation panel, G18 ANN index quantization, G19 multi-tenancy → Phase G above) — this section now covers only items with no current placement:
...
- Native mobile SDK (tracked separately; wait for Python/TS SDK G12/G13 to stabilise first).

These become sensible only once Phase F foundations (F5 horizontal scale, F6 pluggable storage) are stable.
...
quadrantChart
    ...
    "E2 Python SDK (→G12)": [0.4, 0.5]
    "E3 TypeScript SDK (→G13)": [0.5, 0.5]
    ...
    "G3 TTL+scoping+entity graph": [0.55, 0.7]
    "G4 Memory decay+consolidation": [0.5, 0.6]
    "G5 Fuzzy recall 7-stage": [0.6, 0.65]
    "G6 Agent capture hooks": [0.55, 0.75]
    "G7 Provenance verify_result": [0.4, 0.7]
    "G8 Injection scanner+trust": [0.45, 0.7]
    "G9 Leases+signals": [0.5, 0.45]
    "G10 OpenAI shim": [0.25, 0.8]
    "G11 Provider matrix Ollama": [0.3, 0.65]
    "G12 Python SDK": [0.4, 0.7]
    "G13 TypeScript SDK": [0.5, 0.65]
    "G14 Citation panel": [0.35, 0.75]
    "G15 Sub-question decomp": [0.45, 0.65]
    "G16 ACL snippets": [0.3, 0.6]
    "G17 Workspace events": [0.5, 0.55]
    "G18 ANN quantization": [0.55, 0.5]
    "G19 Multi-tenancy storage": [0.75, 0.55]
```
...
**Defer until triggered** (high effort, currently low impact): E6 admin UI, F4 multimodal, F5 horizontal scale, F6 pluggable backends.
...
**Phase E — Surface and integrations**
39. ⬜ F3. GraphRAG / entity-relationship retrieval (item 33) — moved from Phase F.
40. ⬜ G3. TTL, scoping, and entity graph — moved from Phase G.
41. ⬜ E1. Streaming search results (item 27).
42. ⬛ E2. Python SDK — superseded by G12.
43. ⬛ E3. TypeScript SDK — superseded by G13.
44. ⬜ E4. Per-collection access-control policies (item 30).
45. ⬜ E5. Connector and federation architecture (item 15).
46. ⬜ E6. Admin / debug UI (item 29). *A4 prerequisite met; not yet built.*

**Phase F — Advanced positioning**
47. ⬜ F1. Salience and temporal weighting (item 31).
48. ⬜ F2. Semantic memory tiers (item 32).
49. ⬜ F4. Richer multimodal retrieval (item 34).
50. ⬜ F5. Reassess horizontal scaling (item 35).
51. ⬜ F6. Reassess pluggable storage backends (item 36).

**Phase G — World-class differentiation**
52. ⬜ G1. LLM-extracted fact collections.
53. ⬜ G2. Memory mutation history and conflict resolution.
54. ⬜ G4. Memory decay, reinforcement, and consolidation.
55. ⬜ G5. Seven-stage fuzzy recall pipeline.
56. ⬜ G6. Agent-native capture hooks.
57. ⬜ G7. Provenance and verify_result endpoints.
58. ⬜ G8. Prompt injection scanner and trust metadata.
59. ⬜ G9. Leases and signals.
60. ⬜ G10. OpenAI-compatible API shim.
61. ⬜ G11. LLM provider matrix (Ollama, OpenAI).
62. ⬜ G12. Python SDK.
63. ⬜ G13. TypeScript SDK.
64. ⬜ G14. Citation panel and evidence attribution.
65. ⬜ G15. Sub-question decomposition pipeline.
66. ⬜ G16. Permission-aware search snippets.
67. ⬜ G17. Workspace lifecycle events and content graph edges.
68. ⬜ G18. ANN index quantization for large collections.
69. ⬜ G19. Multi-tenancy at storage level.
...
6. ✅ **Phase D** — finish the job contract, export/import, key rotation; the system is operable end-to-end. (All D1–D9 have shipped.)
7. ⬜ **Phase E** — GraphRAG (F3) and entity graph (G3) first (no salience/tier dependency; closes largest architectural gap); then streaming E1, connectors E4/E5, admin UI E6. Phase F salience/tiers/multimodal/scale follow after.
8. ⬜ **Phase G** — world-class differentiation: memory platform (G1–G5), agent integration (G6–G9), ecosystem surface (G10–G13), explainability (G14–G17), scale (G18–G19).
