...
**Status**: Draft — 12-system comparison integrated across all dimensions on 2026-05-21
**Last reviewed**: 2026-05-21 / **Next review**: 2026-10-30
...
|---|---|---|---|---|---|---|
| **Archon** | Standalone local hybrid retrieval + routing server | LanceDB vector + FTS + RRF + cross-encoder rerank; collection routing | REST + MCP server + CLI | Strong local pipeline, routing, crash recovery, tests, and MCP exposure | Single-node embedded LanceDB limits horizontal/concurrent-writer scaling | Keep the retrieval core; add higher-level context, memory, graph, and explainability surfaces |
...
|---|---|---|---|---|---|---|---|---|
| **Archon** | Layered Python service with parser → chunker → embedder → store → reranker → pipeline plus router/server/CLI | Strong local file parsing, chunk config, watch, crash recovery | Strong vector + FTS + RRF + default cross-encoder rerank + collection routing | LanceDB embedded; local-first but single-node | REST, MCP, CLI | Strong tests and local service posture | Collection-level, not agent-memory-native yet | Keep core; add context packets, graph/provenance, lifecycle memory |
...
|---|---|---|---|---:|
| **Archon** | Local file ingestion with parser, chunker, embedding, LanceDB insert, state tracking, watcher, and config-driven chunking. | Strongest local ingest reliability in this field: idempotent file hashes, watch mode, crash recovery, and auto-reindex on chunk-size changes. | No native web/GitHub/YouTube connector layer and no VLM PDF path yet. | 9 |
...
|---|---|---|---|---:|
| **Archon** | LanceDB vector + FTS with RRF, default cross-encoder rerank, context-window retrieval, and multi-collection routing. | Best local search core in the comparison. | Missing HyDE/RAG Fusion, user-facing metadata filters, and graph/provenance stream. | 9 |
...
|---|---|---|---|---:|
| **Archon** | fastembed local embeddings plus fastembed cross-encoder reranker; model configurable globally. | Strong local CPU/GPU-friendly default and dimension guard. | Per-collection model override exists in metadata but is not fully wired. | 8 |
...
|---|---|---|---|---:|
| **Archon** | LanceDB embedded plus collection metadata/state under local runtime directory. | Fast local vector/FTS store with simple deployment. | No storage-level horizontal scaling or concurrent-writer safety. | 7 |
...
|---|---|---|---|---:|
| **Archon** | FastAPI REST, FastMCP server, CLI, Bearer auth. | Strong machine-facing search/API surface. | No web UI/SDK/streaming endpoint yet. | 7 |
...
|---|---|---|---|---:|
| **Archon** | `uv` dev flow, config TOML, API key bootstrap, service install paths, health/status, telemetry, job state. | Strong local daemon operations. | No UI/doctor/update command yet; single-node runtime. | 8 |
...
|---|---|---|---|---:|
| **Archon** | Multi-collection centroid routing, pinned collections, default reranking, crash recovery, MCP search server. | Strong local search server foundation. | Innovation should now move up to context packets, graph/provenance, and memory lifecycle. | 8 |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| **Archon** | Local hybrid search server | 8 | 9 | 9 | 8 | 7 | 7 | 8 | 9 | 8 | 8 | 8 | **89** | Best current local-first retrieval core |
