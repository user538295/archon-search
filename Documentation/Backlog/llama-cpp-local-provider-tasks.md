...
- [ ] **T-2** — Project close-out & acceptance fact-check #tester-role
    ...
    - Duties
        ...
        - Update user/operator-facing docs missed by the team-plan's list (verified gaps, not speculative): [README.md](../../README.md) — HyDE/RAG-Fusion provider list (lines ~50-51) is missing `llama_cpp`; [Documentation/UserManual/30_configuration.md](../UserManual/30_configuration.md) — `[hyde]`/`[rag_fusion]` `provider` table row (~line 162) is missing `llama_cpp` and needs a new `llama_cpp_base_url` row; [Documentation/UserManual/20_wizard.md](../UserManual/20_wizard.md) — the provider walkthrough (~lines 298-355) is missing the `llama_cpp` option/picker flow and the new FE-2 graph-provider wizard step; [Documentation/OperatorGuide/60_graph_operations.md](../OperatorGuide/60_graph_operations.md) — the "Opt-in LLM enrichment" section (~lines 155-175) still documents the old combined `extraction_model = "provider:model"` string format, which BE-4/BE-7 replaced with a discrete `[graph] provider` field + bare `extraction_model` — this is a pre-existing doc/code contradiction this feature must fix, not just an addition.
