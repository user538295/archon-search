...
Ship a minimum end-to-end "stronger routing" slice: (1) a `description_embedding` artifact stored on `CollectionMeta` and persisted in the meta table; (2) an opt-in `routing_strategy = "hybrid"` blend of centroid-cosine and description-embedding-cosine in `MultiCollectionRouter.rank()`; (3) a rank-sensitive eval metric (P@1 / MRR over gold position, scoped to `metric_scope == "routing"` traces) that moves when ranking order changes; (4) expanded routing fixtures that make the metric discriminating; (5) `thresholds.toml` floors encoding Δ ≥ 0 for hybrid vs centroid baseline. Centroid-only stays the default and must reproduce today's ordering byte-for-byte.
...
- MCP: `list_collections` strips `description_embedding` (same as `centroid`); `get_collections_meta` / `get_collection_meta` expose `description_embedding`.
...
- New ADR-07 superseding/extending ADR-04; `BREAKING.md` entry; `tests/eval/README.md` updates; `03_world_class_roadmap.md` B4 checkbox; `archon-search.toml.example`.
...
- `routing_strategy = "centroid"` (default) reproduces today's `rank()` ordering byte-for-byte (regression-pinned by unit test).
...
- A collection whose `embedding_model` field is `""` (legacy default) will always receive centroid-only fallback under hybrid, even if its `description_embedding` was computed with the current model. This is conservative and accepted: the router cannot verify model compatibility without the tag, so it defaults to safe.
...
- `get_collections_meta` MCP payload now includes `description_embedding` per collection, which may be large (e.g. 768 floats). This is accepted for the initial slice: `list_collections` (the lightweight listing tool) strips it, and `get_collections_meta` is used for detailed inspection. Payload trimming (e.g. an `include_embeddings` query param) is deferred to B6.
...
- `MultiCollectionRouter.__init__` gains `strategy: Literal["centroid", "hybrid"] = "centroid"` and `description_weight: float = 0.3`.
- `_score_collections` extended: under `"hybrid"`, per-collection score = `(1 - w) * cos(q, centroid) + w * cos(q, desc_emb)` when description embedding is present AND `col.embedding_model == self._embedding_model` AND `col.embedding_model != ""` (non-empty tag required to verify model compatibility); falls back to `cos(q, centroid)` otherwise (absent embedding, model-mismatch, or empty tag all trigger centroid-only).
- `rank()` calls the extended `_score_collections`; all downstream logic (confidence gate, shortlist slice, unscored fallback) unchanged.

**`archon_search/config.py`**
- `SearchConfig` gains `routing_strategy: str = "centroid"` and `routing_description_weight: float = 0.3`.
...
- `list_collections`: also pops `description_embedding` key (alongside `centroid`).
- `get_collections_meta` / `get_collection_meta`: no change — `description_embedding` is included as a new field.
...
- `_run_router_for_query` gains `strategy: str = "centroid"` and `description_weight: float = 0.3` params; passes them to `MultiCollectionRouter`; returns `list[str]` (full ranked order — all collections are scored because eval fixtures always populate centroids before routing runs; see Task 1.3 fixture precondition assertion).
- `run_eval_suite` maintains **two separate trace lists** alongside the main `traces` list: `centroid_routing_traces` and `hybrid_routing_traces`. For each routing-scope query, one `QueryEvalTrace` is created for the centroid run (added to `centroid_routing_traces`) and one for the hybrid run (added to `hybrid_routing_traces`). The main `traces` list retains only centroid traces for backward-compatible `routing_accuracy` computation. `compute_routing_mrr` / `compute_routing_precision_at_1` are called against each list independently. The `collection_metas` list passed to the hybrid router must be re-fetched from the store after `recompute_collection_meta` to ensure `description_embedding` is populated.
...
| `routing_description_weight` | `float` | `0.3` | Must be in `[0.0, 1.0]` |
...
- **Tests (TDD)** — `tests/eval/test_corpus_contract.py`, `tests/eval/test_fixtures.py`:
  ...
  - Unit: `test_fixture_faq_vocab_distinct_from_other_collections` — using the SHA-256 deterministic embedder from `archon_search/eval/backends.py`, embed the FAQ routing queries and compute centroid similarity against each collection's centroid (derived from the fixture corpus). Assert that the FAQ queries have higher similarity to the `"faq"` centroid than to the `"code"` or `"docs"` centroids. This pins the distinctness property the fixture design relies on.
...
- **Description**:
  - `_run_router_for_query` gains params `strategy: str = "centroid"` and `description_weight: float = 0.3` (both keyword-only); passes them to `MultiCollectionRouter` constructor. Return type stays `list[str]` (full ranked order — since `shortlist_size = len(collections)` and `confidence_threshold = 0.0`, and all fixture collections have centroids, this is the complete score-ordered list).
  ...
  - Extend `_QUALITY_FLOOR_FIELDS` to include `routing_mrr_centroid` (the gating field for this phase).
...
- **Tests (TDD)** — `tests/test_pipeline.py`:
  ...
  - Unit: `test_ingest_re_embeds_description_on_every_ingest` — existing meta has `description = "old desc"`, `described_at_doc_count` does not trigger regeneration; assert `embed_one` is called with `"old desc"` and the result is stored. (Description is always re-embedded to avoid stale state; see Known limitations.)
...
- **Releasable**: `fetch_metadata` round-trips `description_embedding` when the server includes it.
- **Tests (TDD)** — `tests/test_router.py`:
  ...
  - Checkpoint: `uv run pytest tests/test_router.py -x -k "description_embedding or routing_fields"`

#### Task 4.2 — Implement `strategy` + `description_weight` in `MultiCollectionRouter`
- [ ] **File**: `archon_search/router.py`
- **Depends on**: Task 4.1, Task 2.1
- **Description**:
  - `__init__` gains `strategy: Literal["centroid", "hybrid"] = "centroid"` and `description_weight: float = 0.3`; stored as `self._strategy` and `self._description_weight`.
  - `_score_collections` is extended:
    - Under `"centroid"` (default): behavior identical to today — score = `cos(q, centroid)`.
    - Under `"hybrid"`: for each collection with a usable centroid AND a `description_embedding` that is not `None` AND `col.embedding_model == self._embedding_model` AND `col.embedding_model != ""`: score = `(1 - self._description_weight) * cos(q, col.centroid) + self._description_weight * cos(q, col.description_embedding)`. For collections with usable centroid but absent `description_embedding`, model-mismatched `description_embedding`, or `embedding_model == ""`: score = `cos(q, centroid)` (centroid-only per-collection fallback). The "usable centroid" guard (`col.centroid is not None and col.embedding_model == self._embedding_model`) is unchanged and applies first. Note: `embedding_model == ""` (legacy default) is explicitly excluded from the blend — the router cannot verify model compatibility without the tag.
...
- **Releasable**: `MultiCollectionRouter(strategy="hybrid", description_weight=0.3)` blends signals correctly with per-collection fallback; `strategy="centroid"` is byte-for-byte identical to pre-B4.
- **Tests (TDD)** — `tests/test_router.py`:
  - Unit: `test_centroid_strategy_identical_to_pre_b4` — regression pin: construct two `MultiCollectionRouter` instances with identical collections (including some with `description_embedding` set), one with `strategy="centroid"` (new param) and one as `MultiCollectionRouter(..., strategy="centroid")` with default params; assert `rank()` produces identical orderings. This avoids the circular comparison of comparing against a "default router" and instead verifies that the `strategy="centroid"` code path produces the same numeric scores as the pre-B4 `_score_collections` logic (which can be preserved as a separate internal function or verified by hardcoded expected scores with synthetic embeddings).
  ...
  - Unit: `test_model_mismatch_description_embedding_ignored` — collection with `description_embedding` set but `embedding_model` not matching `router._embedding_model`; under hybrid, description embedding is ignored; score = centroid cosine.
  - Unit: `test_empty_embedding_model_falls_back_to_centroid` — collection with `description_embedding` set but `embedding_model == ""` (legacy default); under hybrid, description embedding is ignored and fallback to centroid cosine applies.
...
- **Tests (TDD)** — `tests/test_config.py`:
  ...
  - Unit: `test_routing_description_weight_default` — default is `0.3`.
...
- **Description**:
  ...
  - `mcp.py` `get_collections_meta` and `get_collection_meta`: no change — `description_embedding` is included naturally (it's a new field on `CollectionMeta` defaulting to `None`).
...
- **Tests (TDD)** — `tests/test_routes_route.py`, `tests/test_mcp.py`:
  ...
  - Unit: `test_build_router_default_weight_is_0_3` — config loaded from empty TOML (all defaults); assert `MultiCollectionRouter` is called with `description_weight=0.3`. This catches hardcoded constant bugs in `_build_router`.
  - Unit: `test_list_collections_strips_description_embedding` — mock `pipeline.get_all_collections_meta()` returning a meta with `description_embedding=[0.1]`; assert the MCP payload does NOT contain `"description_embedding"`.
  - Unit: `test_get_collections_meta_includes_description_embedding` — same mock; assert the MCP payload DOES contain `"description_embedding"`.
  - Unit: `test_get_collection_meta_includes_description_embedding` — same mock for single collection.
...
- **Description**:
  - For each routing-scope query in `run_eval_suite`, after recording the centroid `ranked_collections` trace (in `centroid_routing_traces`), run a second call: `hybrid_ranked = await _run_router_for_query(pipeline, q.text, collection_metas, strategy="hybrid", description_weight=0.3)`.
  - **`collection_metas` sourcing**: the `collection_metas` list passed to the hybrid router call must be the same list produced by `recompute_collection_meta` during corpus ingest (after Task 3.2, this now includes `description_embedding`). Concretely, in `run_eval_suite`, `collection_metas` must be re-fetched from the store (via `pipeline.get_all_collections_meta()` or equivalent) **after** `recompute_collection_meta` has been called on each collection, not before. The plan must make this re-fetch point explicit — using a stale list fetched before `recompute_collection_meta` would give all collections `description_embedding=None` and make hybrid identical to centroid.
...
- **Tests (TDD)** — `tests/eval/test_runner.py`:
  ...
  - Unit: `test_collection_metas_are_refetched_after_recompute` — mock `pipeline.get_all_collections_meta()` (or equivalent fetch call) so that: (a) the first call returns a list of metas with `description_embedding=None` (pre-recompute state); (b) the second call returns a list with `description_embedding=[0.1, 0.2, ...]` (post-recompute state). Assert that when `run_eval_suite` constructs the hybrid router, the `initial_metadata` argument it passes contains metas from the second call (non-None `description_embedding`). If the runner uses the pre-recompute list, hybrid scoring silently degrades to centroid-only for all collections.
...
- **Description**:
  - Spawn an agent to discover all documentation in the project and update every file whose content is affected by B4:
    - `Documentation/ADRs/07_description_embedding_hybrid_routing.md` — new ADR recording the hybrid-routing decision, superseding/extending ADR-04 (append-only; do not edit ADR-04). Cover: problem (diffuse centroid), solution (description embedding blend), decision (centroid stays default, hybrid opt-in), consequences (per-collection fallback, eval gate fix).
    - `BREAKING.md` — document: MCP `get_collections_meta` and `get_collection_meta` gain `description_embedding` key (additive, breaking for strict-validating clients); MCP `list_collections` strips it (no change for existing clients); REST `/route` shape unchanged.
    ...
    - `Documentation/Architecture/600_api_reference_or_public_interface.md` — document new `routing_strategy` and `routing_description_weight` config knobs; note MCP `get_collections_meta` shape change.
...
- **Acceptance criteria** (must all pass):
  ...
  - `routing_strategy = "centroid"` (default) produces the same `rank()` ordering as pre-B4 (regression-pinned unit test passes).
  ...
  - MCP `get_collections_meta` response payload contains `description_embedding` (unit test passes).
