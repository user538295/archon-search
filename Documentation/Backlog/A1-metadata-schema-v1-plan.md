...
- **Acceptance criteria** (must all pass):
  ...
  - **Moved to A5c** (was: *During an active reindex of collection A, `POST /ingest` to collection A returns HTTP 503 with `Retry-After: 30` and JSON body `{"error": "store_busy", ...}`. Ingest to a **different** collection succeeds normally.*). The store-layer half — lock + `StoreBusyError` + `Retry-After` ceiling — is pinned by `tests/test_store_lock.py` and was delivered in A1 Task 6.1. The synchronous REST 503 surface is tracked as **A5c** in `Documentation/Backlog/A5-ingest-hardening-plan.md` because A5 already touches the same `/ingest` handler for path validation; bundling avoids a standalone PR. This line intentionally omits a checkbox — A1's done-definition is unaffected; the surface is owned by A5 from here on.
