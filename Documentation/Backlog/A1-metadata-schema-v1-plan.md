...
- **Acceptance criteria** (must all pass):
  ...
  - [ ] During an active reindex of collection A, `POST /ingest` to collection A returns HTTP 503 with `Retry-After: 30` and JSON body `{"error": "store_busy", ...}`. Ingest to a **different** collection succeeds normally. *(store-layer contract — lock + `StoreBusyError` + `Retry-After` ceiling — is pinned by `tests/test_store_lock.py`. Synchronous REST 503 response is bundled into **A5c** of `Documentation/Backlog/A5-ingest-hardening-plan.md` — A5 already touches the same `/ingest` handler for path validation, so combining the changes avoids a standalone PR.)*
