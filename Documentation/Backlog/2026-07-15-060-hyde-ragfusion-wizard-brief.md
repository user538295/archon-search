...
- What is the exact failure mode causing the TOML sections to be absent when the user enters "y"? A post-write assertion in `_write_profile_config` that checks `[hyde].enabled` after writing and logs/raises on mismatch would surface this during the install run itself. This should be implemented as part of this fix to catch any silent truncation or encoding issue.
- Should `_check_provider_deps` in `app.py` be extended to check `import anthropic` for `provider='anthropic'`? This would give an actionable `ConfigError` at server start instead of a silent runtime failure on the first HyDE call. Recommend yes — the existing ollama/openai guards establish the pattern.
- `archon-search[hyde]` and `archon-search[rag_fusion]` are separate optional extras in `pyproject.toml` but both pull `anthropic>=0.40`. Should the wizard install both separately or just `archon-search[hyde]`? Needs a check of the pyproject.toml extras definitions to confirm whether there's a combined `[ai-expansion]` extra.
