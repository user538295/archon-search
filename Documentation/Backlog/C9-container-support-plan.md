...
- **Findings (iterative review)**:
  - Wrapper-vs-inline deviation in `server/app.py`: the plan said "lazy call `get_fasttext_models_dir() / FASTTEXT_MODEL_FILENAME` at the call site"; the implementer introduced a thin private wrapper `_multilingual_model_path()` returning the composition. Deviation is **accepted** because (a) it preserves a single patchable seam consumed by two `tests/server/test_app.py` cases that previously patched the removed `_MULTILINGUAL_MODEL_PATH` module attribute, (b) it consolidates the path-composition logic for the two call sites in `app.py` (`_check_multilingual_deps` and `create_app`), and (c) it matches the existing `_import_fasttext()` pattern already present in the same module. The wrapper is private (`_`-prefixed), pure, and resolves the env on every call — no module-level capture is reintroduced. `pipeline.py::create_pipeline()` keeps inline composition because importing a private symbol from `server/app.py` into the pipeline factory would invert layering. For future plan tasks that say "inline at call site", explicitly allow a `_<name>_path()` wrapper when it preserves an existing patchable seam OR consolidates ≥2 call sites in the same module.
  - Plan TDD list did not require a `create_pipeline()` lazy-resolution test even though Task 2.5 modifies `pipeline.py`. Parity test added in `tests/test_pipeline_optional_reranker.py::test_create_pipeline_uses_lazy_fasttext_models_dir` to lock the contract that `create_pipeline()` with `multilingual=True` reads `ARCHON_SEARCH_DATA_DIR` at runtime. For Task 2.6 and beyond, when a task touches a factory in `pipeline.py` or `server/app.py`, the TDD list MUST include at least one lazy-resolution test per modified factory.
