...
| [`DeveloperGuide/05_mcp_integration.md`](../DeveloperGuide/05_mcp_integration.md) | The 17 MCP tools (D7 added 4 key-management tools), Claude Code wiring, SDK usage. |
...
| [`ADRs/12_local_prose_relations_enrichment_narrowed.md`](../ADRs/12_local_prose_relations_enrichment_narrowed.md) | Why typed prose relations are produced by the local `gliner` engine and the enrichment protocol narrows to community summarisation only (partially supersedes ADR C6's relationship half). |
| [`ADRs/13_graph_device_probe_two_stage_and_memory_guardrail.md`](../ADRs/13_graph_device_probe_two_stage_and_memory_guardrail.md) | Why the graph device probe is two-stage (free availability check, then a validation riding the optional pre-warm) and why the memory lane polices RSS growth under a host-safety ceiling instead of an absolute figure (partially supersedes ADR 12's post-download-validation and `~3.65 GiB` claims). |
...
| [`SecurityGuide/02_authentication_and_keys.md`](../SecurityGuide/02_authentication_and_keys.md) | Bearer auth, default + per-namespace keys, key file permissions, live + legacy key rotation (D7). |
