...
**Connect-failure messages are conditional.** Every connect failure — both job *submission* paths and the `--wait` poll loop — routes through `_server_connect_fail_msg(base_url)` (`cli/_helpers.py`). Given a `base_url` it probes `GET {base_url}/ready`: a `503` whose body carries `checks.models == "pending"` means the server is up and still loading models, so it prints `_SERVER_STARTING_MSG` = `"archon-search is starting up. Please wait for it to finish loading models, then retry."`. Because `/ready` is served by the process itself (not a service manager), this diagnosis works for **every** launch mode, including a foreground `archon-search serve`. Any other outcome — a non-503, a body without `models: "pending"`, or a `ConnectError` on `/ready` too — falls back to `_SERVER_NOT_RUNNING_MSG`. Called without a `base_url`, the helper keeps the legacy path: `_get_service().status().running` (launchd / systemd / Windows), with any probe exception (unsupported platform, missing `launchctl`/`systemctl`) swallowed and the same fallback. Either way the command exits 1.
...
The result drives two read-only surfaces: `GET /status` exposes it as `model_validation: ModelValidationStatus | null` (null while pending), and `GET /ready` maps it to `checks.models: CheckStatus` (strict priority FAIL > WARN > OK; PENDING while unset) without the *validation* result ever gating `ready: bool`. (`ready` is gated on storage plus eager model warm-up — see the `/ready` row in [160_operational_readiness_monitoring_and_reliability.md](160_operational_readiness_monitoring_and_reliability.md); a pending warm-up outranks the validation result and forces `checks.models = "pending"`.) Validation runs once at startup; configuration drift after startup (e.g. a CUDA driver update) is not re-checked until the next restart.
