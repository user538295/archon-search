...
**CSP120** generalised the GBC110 proxy pattern to all CLI write operations. Every write command (`collection add`, `collection remove`, `collection reindex`, `collection reindex-metadata`, `ingest`, `sync`) is now a thin `httpx` proxy that submits a job to the running server and returns immediately with a job ID. The pattern mirrors `migrate_cmd` in `collection.py`: catch `httpx.ConnectError` (prints `"archon-search serve is not running. Start it first."` and exits 1) before the broader `httpx.HTTPError`; on non-202 responses print status + body to stderr and exit 1; on 202 print the job ID and an `archon-search jobs status <job_id>` hint. All write commands add `--api-url` and `--api-key`; all except `remove` add `--wait` (which polls `GET /jobs/{id}` every 2 s, recognises all four terminal statuses, exits 0 on DONE, exits 1 otherwise). The shared poll loop is extracted into `_poll_job(job_id, base_url, headers)` in `archon_search/cli/_helpers.py`.
...
CLI: `--wait` polls `GET /jobs/{job_id}` recognising all four terminal statuses (`DONE`/`FAILED`/`CANCELLED`/`FAILED_EXPIRED`), exiting `0` on `DONE` and non-zero otherwise. A connection failure (server not running) is caught specifically as `httpx.ConnectError`, printing `"Server is not running. Start it first with: archon-search start"` and exiting non-zero — distinct from a reachable server returning a non-2xx status, which prints the generic `"server returned {code}: {text}"`.
