...
**CSP120** generalised the GBC110 proxy pattern to all CLI write operations. Every write command (`collection add`, `collection remove`, `collection reindex`, `collection reindex-metadata`, `ingest`, `sync`) is now a thin `httpx` proxy that submits a job to the running server and returns immediately with a job ID. The pattern mirrors `migrate_cmd` in `collection.py`: catch `_CONNECT_FAIL` (`_helpers.py`'s `(httpx.ConnectError, httpx.ConnectTimeout)` tuple — refused and bound-but-not-listening both mean "server absent"; prints `_SERVER_NOT_RUNNING_MSG` = `"archon-search serve is not running. Start it first with: archon-search serve"` and exits 1) before the broader `httpx.HTTPError`; on non-202 responses print status + body to stderr and exit 1; on 202 print the job ID and an `archon-search jobs status <job_id>` hint. All write commands add `--api-url` and `--api-key`; all except `remove` add `--wait` (which polls `GET /jobs/{id}` every 2 s, recognises all four terminal statuses, exits 0 on DONE, exits 1 otherwise). The shared poll loop is extracted into `_poll_job(job_id, base_url, headers)` in `archon_search/cli/_helpers.py`.

**Connect-failure message inside `_poll_job` is conditional.** Every *submission* path prints `_SERVER_NOT_RUNNING_MSG` unconditionally, but the `--wait` poll loop routes through `_server_connect_fail_msg()` (`cli/_helpers.py`): it asks `_get_service().status().running` and, when the managed service (launchd / systemd / Windows) reports the process alive, prints `_SERVER_STARTING_MSG` = `"archon-search is starting up. Please wait for it to finish loading models, then retry."` instead — the accurate diagnosis when the process is up but has not bound its port yet. Any exception from the service probe (unsupported platform, missing `launchctl`/`systemctl`) is swallowed and falls back to `_SERVER_NOT_RUNNING_MSG`, so a foreground `archon-search serve` — which no service manager tracks — still reads as "not running". Either way the command exits 1.
