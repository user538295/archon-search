...
Built by `archon_search.server.app.create_app`. The lifespan handler connects `SearchStore`, runs `migrate_namespace` + `migrate_acl` + `migrate_per_collection_model`, and then spawns `collection_sync.sync()` on all configured collections (detecting chunk-size changes and triggering auto-reindex when `auto_reindex_on_chunk_size_change = true`) as a background task via `asyncio.create_task`. It is deliberately **not** awaited: uvicorn binds the listening socket only after lifespan startup returns, so awaiting a full corpus sync would leave clients with `ConnectError` for its entire duration instead of a served response. The startup sync is best-effort — a failure logs WARNING and never blocks REST startup — and the task handle is kept on `app.state._startup_sync_task`. The task opens a `SyncJob` of its own (`_start_startup_sync_job`, namespace `default`) and drives it to `DONE`/`FAILED` on every exit path — success, per-collection errors, timeout, cancellation, crash — so the startup sync is visible in `GET /jobs` and, more importantly, leaves a `RUNNING` row behind if the process is killed mid-sync. A job-store write failure there logs a WARNING and the sync runs unmarked; it never fails startup. **Crash-loop guard:** the task is not created at all when `JobStore.crashed_ingest_on_load` is true (loading the job store rewrote an ingest-family job to `FAILED / "process_restart"`, i.e. the previous process died mid-ingest) **or** the sticky suppression sentinel file (`archon-search-sync-suppressed.json`, written by `sync_suppression.py`) already exists under the data dir from an earlier boot — suppression survives a process restart until it is explicitly cleared; the lifespan logs a WARNING naming the manual resume path, leaves `app.state._startup_sync_task` at its `None` default, (re)writes the sentinel, and sets `app.state.sync_result = "suppressed"` instead of `"pending"` (surfaced as `checks.sync: "warn"` on `/ready`, which still answers 200). Without it, an ingest that kills the machine is re-entered unattended on every restart under launchd/systemd auto-restart; without the SyncJob above, the guard would only break a loop whose *first* iteration was a job-backed ingest, never one that dies inside the startup sync itself. `_sync_task` (`routes_sync.py`, via `_clear_degraded_sync_state`) is the only `sync_result` write outside the lifespan: a manual `POST /sync` that completes with no exception and no per-collection errors moves `sync_result` from **either** `"suppressed"` **or** `"failed"` to `"done"`, clears `app.state._startup_sync_failed`, and deletes the sentinel file — clearing is symmetric across both degraded states so `/ready` and `/status` never disagree. **C1**: an `EmbedderCache` (LRU, capacity `config.embedder_cache_size`, default 3) is constructed in the lifespan and injected into `SearchPipeline` so per-collection embedder instances are shared across requests rather than reconstructed per call. Routes are registered as separate `APIRouter`s and grouped by resource. The authoritative wire contract is `GET /openapi.json` (BearerAuth applied to every non-exempt path).
...
`archon_search/jobs/store.py` is the durable state machine for long-running work. `JobStatus` values are `PENDING`, `QUEUED` (D1/D2 — bulk jobs waiting for a scheduler slot), `RUNNING`, `DONE`, `FAILED`, `CANCELLING`, `CANCELLED` — there is no `SUCCEEDED`. The `types.py` module defines `IngestJob` as the base, plus `ReindexJob`, `DeleteJob`, `ExportJob` (D1/D2), `ImportJob` (D1/D2), `CommunityRebuildJob` (GBC110), `SyncJob` (CSP120), and `MetadataReindexJob` (CSP120) subclasses. `SyncJob` and `MetadataReindexJob` carry a `kind: JobKind` field (`JobKind.sync` / `JobKind.metadata_reindex`), emitted as `kind` in `job_to_dict()` via `getattr(job, "kind", None).value`. Jobs are serialised as JSON to `~/.archon-search/archon-search-jobs.json` with atomic-rename writes. Crash recovery: any `RUNNING` or `CANCELLING` job loaded from disk is rewritten to `FAILED` with `error="process_restart"`; `QUEUED` jobs survive crashes (they re-enter the scheduler queue on next tick). When such a rewrite hits an **ingest-family** job (`job_type` `ingest` or `sync` — the two that (re)ingest a corpus), the store exposes it as the read-only `JobStore.crashed_ingest_on_load` property, which the lifespan's crash-loop guard reads (alongside the sticky sentinel file) to suppress the automatic startup sync — for that boot and every boot after until a clean `POST /sync` clears it (see the FastAPI section above). Jobs older than 7 days are evicted — but only terminal jobs (`DONE`, `FAILED`, `CANCELLED`); non-terminal jobs (including `QUEUED`, `PENDING`, `RUNNING`, `CANCELLING`) are never evicted regardless of age (D1/D2 eviction guard). `JobStore.transition(job_id, from_statuses, to_status)` exists and enforces a from-status guard. `update_progress(job_id, processed, total, phase)` updates the `progress: dict | null` field on any job.

Routes that issue jobs: `POST /ingest`, `POST /collections/`, `POST /collections/{name}/reindex`, `POST /collections/{name}/reindex-metadata` (CSP120), `POST /sync` (CSP120), `POST /collections/{name}/export` (D1/D2), `POST /collections/{name}/import` (D1/D2), `POST /graph/{collection}/rebuild-communities` (GBC110). The client then polls `GET /jobs/{job_id}` or lists all jobs with `GET /jobs` (D1/D2). A FAILED export or import job can be resumed via `POST /jobs/{job_id}/resume` (D1/D2).

### CLI write commands as HTTP proxies (CSP120)

**CSP120** generalised the GBC110 proxy pattern to all CLI write operations. Every write command (`collection add`, `collection remove`, `collection reindex`, `collection reindex-metadata`, `ingest`, `sync`) is now a thin `httpx` proxy that submits a job to the running server and returns immediately with a job ID. The pattern mirrors `migrate_cmd` in `collection.py`: catch `_CONNECT_FAIL` (`_helpers.py`'s `(httpx.ConnectError, httpx.ConnectTimeout)` tuple — refused and bound-but-not-listening both mean "server absent or still starting"; prints whatever `_server_connect_fail_msg(base_url)` returns — see the next paragraph — and exits 1) before the broader `httpx.HTTPError`; on non-202 responses print status + body to stderr and exit 1; on 202 print the job ID and an `archon-search jobs status <job_id>` hint. All write commands add `--api-url` and `--api-key`; all except `remove` add `--wait` (which polls `GET /jobs/{id}` every 2 s, recognises all four terminal statuses, exits 0 on DONE, exits 1 otherwise). The shared poll loop is extracted into `_poll_job(job_id, base_url, headers)` in `archon_search/cli/_helpers.py`.

**Connect-failure messages are conditional.** Every connect failure — both job *submission* paths and the `--wait` poll loop — routes through `_server_connect_fail_msg(base_url)` (`cli/_helpers.py`). Given a `base_url` it probes `GET {base_url}/ready`: a `503` whose body carries `checks.models == "pending"` **and** `checks.storage == "ok"` means the server is up and still loading models, so it prints `_SERVER_STARTING_MSG` = `"archon-search is starting up. Please wait for it to finish loading models, then retry."`. Because `/ready` is served by the process itself (not a service manager), this diagnosis works for **every** launch mode, including a foreground `archon-search serve`. A `503` with a usable `checks` object that does *not* match that pattern (e.g. `storage: "fail"`) prints `_SERVER_NOT_RUNNING_MSG` directly. Any outcome where the probe itself is unusable — a `ConnectError`/`ConnectTimeout` on `/ready`, a non-JSON body, or a `"checks": null` payload — carries no signal at all, so it **falls through** to the legacy managed-service path: `_get_service().status().running` (launchd / systemd / Windows), with any exception there (unsupported platform, missing `launchctl`/`systemctl`) swallowed and defaulting to `_SERVER_NOT_RUNNING_MSG`. Called without a `base_url`, the helper goes straight to that managed-service path. Either way the command exits 1.

Two new server endpoints were built for CSP120: `POST /sync` (`routes_sync.py`, backed by `SearchCollectionSync.sync()`) and `POST /collections/{name}/reindex-metadata` (`routes_collections.py`, backed by `SearchStore.reindex_metadata()`). Both follow the `rebuild_communities` route pattern: create job (QUEUED) → transition to RUNNING → return 202 (so the body always reports RUNNING, not QUEUED). A new `jobs status <job_id>` CLI subcommand (`jobs_cmd.py`) provides a one-shot status check matching what the submit-command success message instructs.

**`POST /sync` also 409s for the whole lifespan startup-sync window.** `app.state.sync_lock` is not exclusive to this route: the lifespan backgrounds its own startup `collection_sync.sync()` over all configured collections (see `160_operational_readiness_monitoring_and_reliability.md`) and holds `sync_lock` for that call's entire duration. A `POST /sync` (or `archon-search sync`) arriving before the startup sync finishes gets the same `409` as a genuinely concurrent sync request.

`collection add` no longer writes `archon-search.toml` locally — `POST /collections/` already calls `_maybe_save_config()` server-side during every successful request. Read-only commands (`collection list`, `collection info`) keep the direct-store path and work offline.

### Community rebuild job + CLI proxy (GBC110)

`archon-search graph build-communities <collection>` used to run **in-process**: the CLI opened `GraphStore`/`SearchStore` directly and called `CommunityBuilder.build`, racing with a running server writing the same LanceDB files with no coordination. It is now a thin HTTP proxy (`archon_search/cli/graph_cmd.py`) over `POST /graph/{collection}/rebuild-communities` (`server/routes_graph.py`, function `rebuild_communities`), reusing `cli/collection.py`'s `_resolve_api_key`/`_DEFAULT_API_URL`/`_POLL_INTERVAL_SECONDS`/`_TERMINAL_STATUSES`. The route mirrors `migrate_collection`'s shape: validate graph-enabled (422) and collection-exists (404) → reject an already-in-progress rebuild via the persisted `CollectionMeta.community_rebuild_job_id` guard (409, `"community rebuild already in progress for this collection"`) → `job_store.create_community_rebuild(collection, namespace)` → transition `QUEUED → RUNNING` before returning so the `202` body never reports `QUEUED` → spawn `_community_rebuild_task` into `app.state._background_tasks` → return the full `JobResponse` body.

`_community_rebuild_task` constructs a fresh `CommunityBuilder` and awaits `build(collection, ns)`; success maps to `DONE` with `result={"communities_built": N}`, and `ValueError`/`ImportError`/`RuntimeError` map to `FAILED` with the error string (a missing `leidenalg`/`igraph` install surfaces this way — see `[graph] config section` notes). On every terminal exit the task actively clears `community_rebuild_job_id` on `CollectionMeta` (own `try/except`, swallowed on failure); the route's guard also lazily clears the field when the referenced job is found missing or terminal — this lazy path is what unwedges a collection after a server crash mid-rebuild, since `JobStore._load`'s crash-status flip (above) never touches `CollectionMeta`.

Concurrent rebuilds on the same `(namespace, collection)` — including a `MaintenanceLoop`-triggered GC rebuild (below) — serialise via a **module-level `asyncio.Lock` registry** in `community_builder.py` (`_rebuild_locks: dict[tuple[str, str], asyncio.Lock]`), acquired for the whole `CommunityBuilder.build` call. This lock is deliberately separate from `SearchStore.lock_for` (which `update_collection_meta` also acquires) — holding the ingest lock across the build would self-deadlock the terminal-exit clear, and would leave the GC rebuild path (which acquires no lock of its own) unserialised against the route. Because the GC path never sets `community_rebuild_job_id`, a user request arriving mid-GC-rebuild passes the `409` guard and returns `202`, then its task blocks on the shared lock until the GC rebuild finishes — a `202`-then-block trade-off, not a corruption risk.

CLI: `--wait` delegates to the shared `_poll_job` helper, which polls `GET /jobs/{job_id}` recognising all four terminal statuses (`DONE`/`FAILED`/`CANCELLED`/`FAILED_EXPIRED`), exiting `0` on `DONE` and non-zero otherwise. A connection failure is caught as `_CONNECT_FAIL`, printing `"archon-search serve is not running. Start it first with: archon-search serve"` on submit (or, inside `_poll_job`, the `_server_connect_fail_msg()` variant described above) and exiting non-zero — distinct from a reachable server returning a non-2xx status, which prints the generic `"server returned {code}: {text}"`.
...
**GC-triggered community rebuild (GBC110):** orphan cleanup can trigger `_rebuild_communities_async`, which constructs its own `CommunityBuilder` and calls `build()` — this is fire-and-forget (no `JobStore` entry, unlike the user-triggered `POST /graph/{collection}/rebuild-communities` route above) but now serialises against a concurrent user-triggered rebuild via the same module-level rebuild-lock registry in `community_builder.py`, closing the race where both paths could write the community table at once.
...
The result drives two read-only surfaces: `GET /status` exposes it as `model_validation: ModelValidationStatus | null` (null while pending), and `GET /ready` maps it to `checks.models: CheckStatus` (strict priority FAIL > WARN > OK; PENDING while unset) without the *validation* result ever gating `ready: bool`. (`ready` is gated on storage, eager model warm-up, and the lifespan's startup collection sync — see the `/ready` row in [160_operational_readiness_monitoring_and_reliability.md](160_operational_readiness_monitoring_and_reliability.md); a pending warm-up outranks the validation result and forces `checks.models = "pending"`, and a running startup sync forces `ready: false` with its own `checks.sync = "pending"`.) Validation runs once at startup; configuration drift after startup (e.g. a CUDA driver update) is not re-checked until the next restart.
