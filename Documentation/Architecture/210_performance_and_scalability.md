...
| Graph NER ingest throughput          | Median of 3 `ingest_directory` runs over `tests/eval/corpus/docs/` (17 pinned files), asserted ≤ `THROUGHPUT_BASELINE_MS * REGRESSION_MULTIPLIER` | `tests/test_graph_ner_real_artifact_lane.py::test_graph_ner_throughput_within_budget` | **Hard CI gate** in the `graph_real_artifact` lane — real GLiNER checkpoint, fastembed stubbed. The baseline is a provisional dev-machine capture, so the multiplier is wide (catches catastrophic regressions only) until the first green CI run replaces it. |
