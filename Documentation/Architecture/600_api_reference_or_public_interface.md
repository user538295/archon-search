...
| Method | Path | Purpose | Request schema | Response schema |
| --- | --- | --- | --- | --- |
| POST | `/sync` | **CSP120-BE3** — Submit a sync job for all collections visible in the caller's namespace; returns `202` immediately with `{job_id, status: "RUNNING"}`. Returns `409` (`{"detail": "sync already in progress"}`) when an active sync job already exists for the namespace. No request body. | — | `202 JobResponse` / `409` (already in progress) |
