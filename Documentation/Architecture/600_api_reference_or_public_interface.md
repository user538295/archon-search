...
| `provider_warnings` | `list[str]` | Human-readable warnings (missing ONNX provider, probe failure, `"validation timed out after {N}s"`). Empty list when clean. **2026-08-19-030 / MGE** — also carries graph prose-extraction engine state when `[graph].enabled = true`, prepended ahead of the probe warnings on every return path. Every entry here is **operator-actionable**, and that is the contract: `graph_ner_status()` reports either the `[graph]` extra being absent (remedy: install it) or `"graph NER model presence could not be determined"` (remedy: read the logged probe error). It does **not** report model-artifact state — the checkpoint is fetched lazily on first load and an artifact failure degrades that ingest instead of surfacing here. There is no English-only entry: the pinned checkpoint is multilingual. |
...
`StatusResponse` gains `warmup_result: WarmupResult | null`, distinct from `model_validation` above — it tracks the **eager warm-up task**, not the model-validation probe. `WarmupResult` is a named enum schema in the generated OpenAPI contract (`anyOf[$ref WarmupResult, null]`), with members `"pending"` (set on `app.state.warmup_result` before the warm-up task starts, so there is no ready-looking gap between lifespan startup returning and the field appearing), `"done"` (both the embedder cache and the reranker cross-encoder finished loading), `"failed"` (the task raised, or ran past the `_EAGER_WARMUP_TIMEOUT_SECONDS` terminal ceiling — 600s — without finishing; a failed warm-up is non-fatal, the server falls back to lazy loading on the first real query). `null` when `[database].eager_load_embedders = false` — no warm-up task is ever started. This is the same value `GET /ready` reads via `_warmup_pending()` to decide `checks.models = "pending"` and the 503 status (see the `routes_ready.py` row above); a **terminal** `"failed"` value is read separately by `_model_check_status`, which reports `checks.models: "fail"` in that case — even if the unrelated, cheaper `model_validation` resolvability probe reported clean — mirroring the equivalent `checks.sync: "fail"` state for a failed startup sync. Neither `"fail"` gates `ready`.

The `sync_result` field added to `GET /status`:

`StatusResponse` gains `sync_result: StartupSyncResult | null` — the startup-sync counterpart to `warmup_result` above, mirroring its `"pending"`/`"done"`/`"failed"` shape (plus one member of its own, `"suppressed"`) but tracking the lifespan's **startup collection sync**, not model loading. `StartupSyncResult` is a named enum schema in the generated OpenAPI contract, with members `"pending"` (set on `app.state.sync_result` before the sync task starts), `"done"` (the sync completed with no per-collection errors), `"failed"` (the sync raised, was cancelled during shutdown, completed with a non-empty `SyncResult.errors`, or ran past the `_STARTUP_SYNC_TIMEOUT_SECONDS` terminal ceiling — 1800s — without finishing), `"suppressed"` (collections are configured, but the lifespan's crash-loop guard refused to start the sync task at all — either because loading the job store rewrote an ingest-family job to `FAILED / "process_restart"` **this** boot (the previous process died mid-ingest), **or** because the sticky suppression sentinel file (`archon-search-sync-suppressed.json`, `sync_suppression.py`, under the data dir) already existed from an **earlier** boot; the index may be stale until an operator runs `POST /sync` or `archon-search sync`). The startup sync opens a `SyncJob` of its own for exactly this reason, so a crash *inside the startup sync* leaves the same marker and the guard arms itself rather than only breaking a loop that began with a client-submitted ingest — and, via the sentinel, **stays armed on every subsequent restart**, not just the one boot immediately after the crash. `"suppressed"` is degraded-but-ready: no sync task is created, so `_startup_sync_pending()` is false and `/ready` stays 200, reporting the state as `checks.sync: "warn"`. Neither `"suppressed"` nor `"failed"` is terminal for the process: a `POST /sync` that completes with no exception and no per-collection errors moves `sync_result` from **either** state to `"done"` and deletes the sentinel (`routes_sync._clear_degraded_sync_state`) — clearing is symmetric on purpose, since `GET /ready` reads `_startup_sync_failed` and `GET /status` reads `sync_result`, and the two must never disagree. No other `POST /sync` outcome writes `sync_result`. `null` when no collections are configured (`pinned_collections` and `[collections]` both empty) — no startup sync task is ever created. This is the same value `GET /ready` reads via `_startup_sync_pending()` / `_sync_check_status()` to decide `checks.sync` and, while pending, the 503 status (see the `routes_ready.py` row above).
...
| Field | Type | Meaning |
|---|---|---|
| `key_available` | `bool` | `true` when the configured provider's required API key is set at call time (`ANTHROPIC_API_KEY` for `"anthropic"`, `OPENAI_API_KEY` for `"openai"`; always `true` for `"ollama"`, `"claude_cli"`, and `"llama_cpp"` — none of these need a key). |
| `provider` | `str` | Active LLM provider name: `"anthropic"` (default), `"ollama"`, `"openai"`, `"claude_cli"`, or `"llama_cpp"` — mirrors `[hyde].provider` in config. **G10 / LLCP** |
...
| Method | Path | Purpose | Request schema | Response schema |
| --- | --- | --- | --- | --- |
| GET | `/ready` | Unauthenticated readiness probe for load balancers. `ready: bool` is `true` (200) when **all three** hold: `SearchStore.ping()` succeeds, eager model warm-up is not still pending, and the lifespan's startup collection sync is not still running; `false` (503) otherwise. **D6** — `checks` gains a `models: CheckStatus` field driven by the background model-validation result; that result is informational and does **not** affect `ready` or the HTTP status (a `models: "fail"` still answers 200, since the lazy-load contract means a search may still succeed). The one model-side condition that *does* gate readiness is eager warm-up: when `[database].eager_load_embedders = true` and `app.state.warmup_result == "pending"`, `checks.models` reads `"pending"` and the endpoint answers 503, because the first search would otherwise block on ONNX construction. A **terminal** `app.state.warmup_result == "failed"` (the warm-up task raised, or ran past `_EAGER_WARMUP_TIMEOUT_SECONDS`) is checked separately, outranking `model_validation`: `checks.models` reads `"fail"` even if `model_validation`'s cheaper resolvability probe reported clean — mirroring `checks.sync: "fail"` below — but, like `sync: "fail"`, this does NOT gate `ready` or the HTTP status; only the `"pending"` state does. The **third** gating condition is independent of `eager_load_embedders`: while `app.state._startup_sync_task` (the lifespan's backgrounded `collection_sync.sync()` over all configured collections) has not finished, `ready` is `false` and the endpoint answers 503 — the index may still be (re)building. `checks` gains a `sync: CheckStatus` field for this condition, resolved FAIL > WARN > OK after the pending check: `"pending"` while the startup sync task is still running, else `"fail"` if the sync either raised or completed with per-collection errors in `SyncResult.errors` (`app.state._startup_sync_failed`), else `"warn"` if the crash-loop guard suppressed the sync (`app.state.sync_result == "suppressed"` — no task ever ran, so a bare `"ok"` would be an all-clear over a potentially stale index), else `"ok"`. `sync: "fail"` and `sync: "warn"` are informational only, like `models: "fail"`/`models: "warn"` — it does NOT gate `ready` or the HTTP status; `/ready` answers 200 with `checks.sync: "fail"` since a corrupted collection must not wedge the pod's readiness forever. | — | `ReadinessResponse` (`schemas.py`) |

`checks.models` mapping (`_model_check_status`, `routes_ready.py` — strict priority: eager-warm-up PENDING > eager-warm-up FAILED > `model_validation` FAIL > WARN > OK):
...
| `warn` | Both `model_validation` probes passed but `provider_warnings` is non-empty — an ONNX provider fell back, **or** (2026-08-19-030 / MGE) `[graph].enabled = true` with the `[graph]` extra not importable, or its presence undeterminable. The graph cause is not search-affecting: it degrades prose entity extraction only. Because `provider_warnings` is the sole input to this grade, nothing permanent and unactionable is ever put there — that channel was removed with `provider_notes` and deliberately has no successor (ADR 12), so `warn` here always names something an operator can clear. |
...
| POST | `/collections/{name}/reindex-metadata` | **CSP120-BE4** — Enqueue a `MetadataReindexJob` to backfill metadata fields (`file_type`, `updated_at`, `ingested_by`) and optionally normalise `indexed_at`/`updated_at` timestamps to fixed-width UTC. Returns `202` immediately. Two-stage 404 guard: (1) collection must be in `_all_collection_paths(config)`, (2) `get_collection_meta()` must return a valid record. Returns `409` when a metadata-reindex job is already active for this collection (via `CollectionMeta.metadata_reindex_job_id` guard, lazily cleared when stale). On DONE, `result` carries all 5 `ReindexResult` fields: `processed`, `updated`, `skipped`, `ts_normalized`, `warnings`. | `{"dry_run": bool = false, "normalize_timestamps": bool = true}` | `202 JobResponse` / `404` (unknown collection) / `409` (duplicate submission) |

### `routes_sync.py`

| Method | Path | Purpose | Request schema | Response schema |
| --- | --- | --- | --- | --- |
| POST | `/sync` | **CSP120-BE3** — Submit a sync job for all server-configured collections; returns `202` immediately with `{job_id, status: "RUNNING"}`. Runs only `SearchCollectionSync.sync()` — distinct from `POST /maintenance/trigger` (which runs a full maintenance pass). Returns `409` (`{"detail": "sync already in progress"}`) when `app.state.sync_lock` is already held. No request body. On DONE, `result` carries all 6 `SyncResult` fields: `added`, `removed`, `unchanged`, `errors`, `skipped`, `updated`. Note: sync is namespace-blind in v1 — all configured collections are synced regardless of caller namespace (see Known Limitations in the CSP120 team plan). | — | `202 JobResponse` / `409` (already in progress) |
...
| GET | `/jobs` | **D1/D2** — List all jobs visible to the caller's namespace, sorted by `created_at` descending. Supports cursor-based pagination and filtering by status, job kind, and (D2) source. Not every job here was submitted by a client: the lifespan's startup collection sync opens its own `SyncJob` in the `default` namespace (`source: "user"`) so a crash mid-sync leaves a `process_restart` marker for the crash-loop guard — see `sync_result` above. | Query: `status=` (repeatable, any `JobStatus` value), `kind=` (repeatable: `ingest`, `reindex`, `delete`, `export`, `import`, `migration`), `source=` (repeatable: `user`, `backup`), `limit=50` (max 200), `cursor=` (job_id of the last item from the previous page). | `{"items": list[JobResponse], "next_cursor": str \| null, "total": int}` |
...
| Method | Path | Purpose | Request schema | Response schema |
| --- | --- | --- | --- | --- |
| POST | `/backup/trigger` | **D2** — Trigger an immediate backup pass for every collection in the caller's namespace. Returns `202` with `BackupTriggerResponse` listing the `queued` items and `skipped` collections (each with `reason` in `excluded` / `already_active` / `already_queued`). Jobs are created with `source="backup"` and dispatch via the standard `JobScheduler` behind any user-sourced bulk jobs. Archives land in `{backup.output_dir}/{namespace}/{collection}.backup.{timestamp}.tar.gz`. | — | `BackupTriggerResponse` — `{"queued": list[{"collection": str, "job_id": str}], "skipped": list[{"collection": str, "reason": str}]}` |
...
| `hyde` | `bool` | `false` | **C4** — Use HyDE hypothesis embedding for the ANN lookup leg. Requires `[hyde] enabled = true` in config and the provider-appropriate package installed (`archon-search[hyde]` for Anthropic, `archon-search[ollama]` for Ollama, `archon-search[openai-provider]` for OpenAI; `llama_cpp` needs no extra — `httpx` is a core dep). Provider and required API key are controlled by `[hyde].provider` in config (default: `"anthropic"`). Falls back silently to the standard query embedding on any failure; `hyde_applied: false` in the response reflects the fallback. **G10 / LLCP** |
...
| POST | `/graph/{collection}/rebuild-communities` | **GBC110** — Enqueue an async Leiden community rebuild for a collection, mirroring `POST /collections/{name}/migrate`'s create-job-then-`202` shape. Namespace is derived from the Bearer token (`request.state.namespace`). Returns `422` when `[graph] enabled = false` (verbatim `routes_graph.py` detail, same string as the other guards in this table) or when `graph_store` failed to connect at startup despite `enabled=true`. Returns `422` when `?namespace=` is provided but does not match the token's namespace (`"namespace mismatch: token authorises '{ns}', but ?namespace='{query_ns}' was requested"`). Returns `404` when the collection does not exist in the caller's namespace. Returns `409` (`"community rebuild already in progress for this collection"`) when `CollectionMeta.community_rebuild_job_id` points at a still-active job — a stale id (job missing or terminal) is lazily cleared and the request proceeds. Concurrent rebuilds for the same `(namespace, collection)` — including a `MaintenanceLoop` GC-triggered rebuild — serialise via a module-level lock registry in `community_builder.py`; this does not block ingest on the collection. | `namespace?: str` (default `"default"`) — optional explicit namespace confirmation; must match the Bearer token's namespace or the request is rejected with `422`. | `202` — full `JobResponse` body (`job_to_dict(running_job)`), `status: "RUNNING"` (never `"QUEUED"`/`"PENDING"` — the route transitions before returning). On `DONE`, `GET /jobs/{id}` reports `result: {"communities_built": N}`. On `FAILED` (e.g. missing `leidenalg`/`igraph`, or zero graph nodes), `error` carries the message. |
...
- The 5 embedder-resolving tools (`search`, `search_with_context`, `explain`, `ingest_file`, `ingest_directory`) each catch a wedged/still-loading embedder-cache load (`EmbedderNotReadyError`) explicitly, ahead of their generic `except Exception`, and return `{"error": "service unavailable: embedding model still loading", "code": "embedder_not_ready"}` — a retryable condition, distinct from `"internal_error"`. Not covered by the app-level `EmbedderNotReadyError` handler used on REST (see `140_error_handling_strategy.md`): the `/mcp` sub-app builds its own middleware stack.
...
| `wizard` | — | Interactive setup wizard: choose a profile, configure optional features, download models, register and start service. Asks interactive questions for multilingual support, code enrichment, reranker, filesystem watcher, telemetry, eager loading, routing strategy, log format, log-to-stderr (when json selected), and GPU confirmation; prompts for AI query expansion (HyDE/RAG Fusion) with provider selection (Anthropic/OpenAI/Ollama/Claude CLI/llama.cpp). All questions are skippable with flags; `--non-interactive` suppresses all prompts and uses defaults. Prompt order: multilingual → profile → GPU confirm → licenses → optional features → summary → pre-warm → graph accelerator offer (only when the two-stage device probe validates one; otherwise CPU is written silently) → service install. Prints a "Next steps" block (including `--top-k` hint and full API key with source label) after a successful install. Warns before overwriting hand-edited config keys on re-run. See `archon_search/install/` and `archon_search/profiles.py`. | `--profile {minimal,balanced,max}`, `--multilingual/--no-multilingual`, `--skip-preload`, `--force`, `--delete-db`, `--accept-jina-license`, `--accept-fasttext-license`, `--dry-run`, `--non-interactive`, `--config`, `--code / --no-code`, `--watch / --no-watch`, `--telemetry / --no-telemetry`, `--eager-load / --no-eager-load`, `--no-reranker`, `--routing-strategy {centroid,hybrid}`, `--log-format {text,json}`, `--disable-gpu`, `--host TEXT`, `--port INTEGER (1–65535)`, `--db-path PATH`, `--log-level {DEBUG,INFO,WARNING,ERROR,CRITICAL}`, `--log-to-stderr`, `--top-k INTEGER (1–100)`, `--telemetry-retention-days INTEGER (≥1)`, `--enable-hyde`, `--enable-rag-fusion`, `--server-key HEX_KEY (lowercase hex ≥32 chars)`, `--graph-providers TEXT` (comma-separated ONNX execution providers for the graph engine; empty string forces CPU) |
...
| `ingest` | — | **FE-5** — Submit an async ingest job to the running server via `POST /ingest` (`cli/ingest.py`). **`--path` is required** (exits 1 with `Error: --path is required.` when omitted). The path is resolved to an absolute path before submission. Collection name defaults to `path_to_collection_name(path)` (full filename, e.g. `report.pdf` → `"report_pdf"`). Server must be running — exits 1 on `ConnectError`; the message is `"archon-search serve is not running. Start it first with: archon-search serve"` or `"archon-search is starting up. Please wait for it to finish loading models, then retry."` (probe-based; for a custom `--api-url`, probe failure always yields the not-running message — S530). Non-202 responses exit 1 with status code and body. `--wait` polls `GET /jobs/{id}` until terminal status and prints `Ingest complete for '<name>'.` on DONE; exits 1 on FAILED. Ctrl-C during polling prints `Polling stopped` and exits 0. | `--path` (required), `--collection`, `--wait`, `--api-url`, `--api-key` |
| `sync` | — | **FE-6** — Proxies to `POST /sync`; submits a server-side sync job for all configured collections (`cli/sync.py`). Server must be running — exits 1 on `ConnectError`. `--wait` polls `GET /jobs/{id}` until terminal and prints `Sync complete.` on DONE; exits 1 on FAILED. 409 (already in progress) prints the detail and exits 1 — this also fires for the **entire lifespan startup sync window**: `POST /sync` holds `app.state.sync_lock`, and the lifespan's own backgrounded startup `collection_sync.sync()` acquires that same lock for its full duration, so a `sync` invoked before startup sync finishes 409s rather than queueing. | `--wait`, `--api-url`, `--api-key` |
| `collection` | `list` | List collections from the store (`cli/collection.py`). | `--config` |
| `collection` | `add <path>` | **CSP120 FE-4** — Proxy to `POST /collections/`; the server writes the path to `archon-search.toml` server-side via `_maybe_save_config()` (the CLI no longer writes TOML locally). Prints the job ID and the server-derived collection name. `--wait` polls to DONE. Server must be running. | `--wait`, `--api-url`, `--api-key` |
| `collection` | `remove <name>` | **FE-8** — Proxy to `DELETE /collections/{name}`; rejects pinned-only (409) and lock-held (503). Requires the server to be running. | `--api-url`, `--api-key` |
| `collection` | `info <name>` | Print collection metadata. | `--config` |
| `collection` | `reindex <name>` | **CSP120 FE-2** — Proxy to `POST /collections/{name}/reindex`; submits a reindex job and prints the job ID. `--wait` polls to DONE. Server must be running. | `--wait`, `--api-url`, `--api-key` |
| `collection` | `reindex-metadata <name>` | **CSP120** — Proxies to `POST /collections/{name}/reindex-metadata`. Backfill metadata fields (`file_type`, `updated_at`, `ingested_by`) on an existing collection without re-ingesting. When `--normalize-timestamps` (default ON) rewrites `indexed_at` and `updated_at` to fixed-width UTC (`YYYY-MM-DDTHH:MM:SS.ffffffZ`) for any row not already in canonical form — required before date-range filters return correct results on pre-A2 collections. `--dry-run` submits a dry-run job; use `--wait` to see the counts (`processed`, `updated`, `skipped`, `ts_normalized`) on completion. | `--normalize-timestamps / --no-normalize-timestamps`, `--dry-run`, `--wait`, `--api-url TEXT`, `--api-key TEXT` |
...
| `graph` | `build-communities <collection>` | **GBC110** — Proxies to `POST /graph/{collection}/rebuild-communities` (`cli/graph_cmd.py`), replacing the old in-process `CommunityBuilder`/`GraphStore`/`SearchStore` call path (which raced with a running server writing the same LanceDB files). Prints the returned `job_id` and namespace, e.g. `"Community rebuild job submitted: job-abc (namespace: default)"`, and exits 0. `--wait` polls `GET /jobs/{job_id}` until a terminal status (`DONE`/`FAILED`/`CANCELLED`/`FAILED_EXPIRED`), exiting 0 on `DONE` and non-zero otherwise; on completion prints `"Community rebuild complete: N communities built (namespace: {ns})."`. A connection failure is caught as `_CONNECT_FAIL` (`httpx.ConnectError` or `httpx.ConnectTimeout`), printing `"archon-search serve is not running. Start it first with: archon-search serve"` — or, when a `GET /ready` probe answers `503` with `checks.storage == "ok"` and either `checks.models == "pending"` or `checks.sync == "pending"` (the server is up and still loading models or still running its startup collection sync), `"archon-search is starting up. Please wait for it to finish loading models, then retry."` (falling through to the OS service-manager status when the probe itself is unusable) — and exiting non-zero; any other non-202/200 response prints `"server returned {code}: {text}"`. No `--config`/local `[graph] enabled` pre-check (removed — the server enforces it via the route's `422`). **Brief 2026-07-15-130** adds `--namespace` / `-n` (default `"default"`), forwarded as `?namespace=` to the route; a token for a different namespace returns `422` (use the correct `--api-key` for the target namespace). | `collection: str`, `--namespace TEXT` / `-n TEXT`, `--wait`, `--api-url TEXT`, `--api-key TEXT` |
...
| `backup` | — | **D2** — Backup Click group. Bare invocation prints help. `--now` calls `POST /backup/trigger` and prints each queued item as `  {collection} → {job_id}` plus skipped collections with reason; `--wait` polls each job to DONE/FAILED/FAILED_EXPIRED, printing `{collection}: DONE`, `{collection}: CANCELLED`, or `{collection}: FAILED — {error}` for each terminal job. **E0b**: `--timeout SECONDS` (default 300) added; on timeout exits 0 and prints collection names + recovery hint to stderr; exits 2 on FAILED or FAILED_EXPIRED (changed from exit 1). | `--now`, `--wait`, `--timeout SECONDS`, `--api-url TEXT`, `--api-key TEXT` |
...
| `jobs` | `status <job_id>` | **CSP120 FE-3** — One-shot job status check. Calls `GET /jobs/{id}` once and prints `job_id`, `status`, `collection`, `created_at`, `progress` (if non-null), `error` (if FAILED/FAILED_EXPIRED). Exit 0 for DONE and in-progress states (PENDING/QUEUED/RUNNING/CANCELLING); exit 1 for FAILED/FAILED_EXPIRED/CANCELLED. Prints `"Job not found: {job_id}"` and exits 1 on 404. | `--api-url TEXT`, `--api-key TEXT` |
