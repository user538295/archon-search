...
| `graph_mode` | `"naive" \| "local" \| "global" \| null` | `null` | **E1c** — When non-null and `[graph] enabled = true`, uses graph-aware retrieval for the explain pipeline. `"naive"`: expands query via first-degree graph neighbours, then standard retrieval with per-candidate `GraphProvenance`. `"local"`: fetches community-representative chunks for matched entities. `"global"`: aggregates community-representative chunks across top-N communities. Mutually exclusive with `collections` (multi-collection) — returns `422` if both are set. When `graph_mode` is non-null, HyDE is suppressed regardless of the `hyde` field value. Returns `422` when `graph.enabled = false`. |
...
- **E1c**: `graph_mode_applied: "naive" | "local" | "global" | null` — set to the mode the pipeline attempted when `graph_mode` was requested (even when no graph candidates were found). `null` when `graph_mode` was not set.
