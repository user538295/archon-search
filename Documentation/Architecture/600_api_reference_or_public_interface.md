...
Returns `404` when the collection is not visible to the caller's namespace; `503` when meta lookup fails. Pipeline stage exceptions (embedder, store, reranker) return `500` with a plain-text body `Internal Server Error` (Content-Type `text/plain`) — the route bare-re-raises and Starlette's `ServerErrorMiddleware` renders the default response, so this is **not** a JSON envelope and callers must not `.json()`-parse the 500 body. A hung pipeline call returns `504` with `{"detail": "Search timed out"}` after ~30 s. That budget covers the search only: because `ModelReranker` builds its ONNX cross-encoder on first use, the handler awaits `pipeline.warmup_reranker()` *before* starting the timer whenever `pipeline.reranker_is_warm` is `False`, so a cold cross-encoder no longer turns the first request after startup into a `504` (S184). The request still waits for that one-off load unless `[database].eager_load_embedders = true` already warmed it at startup. `200` with `results: []` means the pipeline ran successfully but found no matching documents. A malformed `filters` object returns `422`.
