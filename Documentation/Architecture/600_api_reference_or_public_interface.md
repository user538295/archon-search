...
| `provider_warnings` | `list[str]` | Human-readable warnings (missing ONNX provider, probe failure, `"validation timed out after {N}s"`). Empty list when clean. **2026-08-19-030** — also carries **actionable** graph NER model state when `[graph].enabled = true`, prepended ahead of the probe warnings on every return path: `en_core_web_sm` neither installed nor provisioned under the data dir, present but incompatible with the installed spaCy (a different remedy — re-provision), the `[graph]` extra itself missing, or `"graph NER model presence could not be determined"`. |
| `provider_notes` | `list[str]` | **2026-08-19-030** — informational, permanent, not operator-actionable. Currently: the English-only disclosure for `[database].multilingual = true` deployments. Deliberately separate from `provider_warnings` because `checks.models` on `GET /ready` grades that field, and a permanent note there would pin the check to `warn` for the life of the deployment with no action that clears it. Additive field; existing clients ignoring it are unaffected. |
