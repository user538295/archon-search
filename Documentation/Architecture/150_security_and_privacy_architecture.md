...
**Threat-model rationale for unauthenticated `/ready`**: `/ready` is a readiness probe — it answers whether the service's storage layer is connected, any eager model warm-up has finished, and the lifespan's startup collection sync has finished. It returns `{ready: bool, checks: {storage: "ok"|"fail", models: "pending"|"ok"|"warn"|"fail", sync: "pending"|"ok"|"warn"|"fail"}}` (the D6 `checks.models` field reports background model-validation state; `checks.sync` reports the startup-sync gate — its `"fail"` state, unlike `models: "fail"`, is also purely informational) and nothing more: no document counts, no collection names, no version string, no user data. An attacker who can reach the loopback socket already has more sensitive information (the process is running, port is open) than `/ready` reveals. Supervisor tooling (e.g. a load-balancer health check, an install script waiting for warm-up) needs this signal without holding the API key. Withholding it would force the API key into infrastructure-level probing scripts, which is a worse security posture than exposing a no-data probe. `/health` (liveness) and `/ready` (readiness) are intentionally separate endpoints with different shapes — `/health` returns `{status, version}` and is never `503`; `/ready` returns `{ready, checks}` and returns HTTP 503 when the storage layer is unavailable, when an eager model warm-up is still running, or while the lifespan's startup collection sync is still in progress.
...
While a reindex holds a collection's per-collection ingest lock, `POST /jobs/ingest`, `POST /collections`, and `DELETE /collections/{name}` pre-acquire that lock in the request handler (all three via `server/_ingest_lock.py::acquire_collection_lock_or_503`) and, on a 30s acquisition timeout, return HTTP 503 with `Retry-After: 30` synchronously (rather than a 202 followed by a failed job). Ingest into a different collection is unaffected. The MCP `ingest_file` / `ingest_directory` tools surface the same condition as `McpErrorResponse(code="store_busy")`. Note: `POST /collections/` returns a standard `{"detail": "store busy; retry in N seconds"}` body (`ErrorDetail` shape); `POST /jobs/ingest` returns `{"error": "store_busy", "detail": ...}` (normalisation deferred).

The same `lock_for(name)` also guards the check-then-act inside the store: `drop_collection` and `rename_collection` hold it across the `list_tables` existence/conflict check **and** the `drop_table`/`rename_table` call, so a concurrent drop/rename/ingest cannot interleave at the awaits; `ingest_chunks` holds it across its write plus the collection-meta update. Because `asyncio.Lock` is **not** reentrant, a route that already pre-acquired the lock must say so — `drop_collection` and `ingest_chunks` take a private `_locked_by_caller: bool = False` for exactly that, and `DELETE /collections/{name}` passes `_locked_by_caller=True`. `rename_collection` has no pre-holding caller and always acquires internally.
