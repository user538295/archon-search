...
| `pipeline.get_collection_meta(...)` raises any exception (router/meta-lookup failure — `/route`, `/search` when collection metadata is unavailable) | `503` | `routes_search.py:67-71` (catches `Exception`, returns `JSONResponse({"detail": "service unavailable"}, status_code=503)`) | none — `routes_search.py` does not import or call `TelemetryEntry.from_error`; no telemetry is enqueued on this path |
| Store lock contention — `StoreBusyError` raised by an ingest/reindex runner when the LanceDB write lock is already held (`/ingest/*`, `/reindex/*`) | `503` + `Retry-After` | ingest/reindex job runners (A1) | none |
