...
**Model warm-up is outside the `/search` timeout budget (S184).** The ~30 s budget (`_SEARCH_TIMEOUT_SECONDS = 30.0` in `routes_search.py`) covers the search itself, not one-off model initialisation. `ModelReranker` builds its ONNX cross-encoder on the *first* `predict` call, so before entering `asyncio.wait_for` the handler checks `pipeline.reranker_is_warm` and, when it is `False`, awaits `pipeline.warmup_reranker()` — previously that load happened *inside* the budget and could consume it, turning the first search after a cold start into a `504`. Warm-up failure is non-fatal: it logs a WARNING (`reranker warm-up failed; continuing with search`) and the search proceeds. Setting `[database].eager_load_embedders = true` performs the same warm-up in the app lifespan (also WARNING-on-failure), so no request pays the load at all.
