...
| `GET /ready` storage unavailable | `503` | `routes_ready.py` — returns `ReadinessResponse` body (`{"ready": false, "checks": {"storage": "fail", "models": ...}}`; the D6 `checks.models` field is present but does not affect the 503). This is an **expected "not-ready-yet" state**, not a pipeline error. The body is `ReadinessResponse`, never `{"detail": ...}`. Contrast with 503 from `routes_search.py` which uses `JSONResponse({"detail": "service unavailable: metadata store could not be reached", "code": "metadata_store_error"}, status_code=503)` on meta-lookup failure. | n/a (pre-query; not logged) |
