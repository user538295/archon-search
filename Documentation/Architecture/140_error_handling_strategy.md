...
2. **Propagate typed exceptions.** Domain layers raise `ValueError`, `KeyError`, `RuntimeError`, `NotImplementedError`, `ConfigError`. (`asyncio.TimeoutError` arises in the request path from `asyncio.wait_for(...)` in `routes_route.py` and `routes_search.py`; it is not a typed domain exception raised by lower layers.) Route handlers translate these to HTTP status codes — exceptions never leak as raw 500s when a typed mapping exists.
...
| `pipeline.get_collection_meta(...)` raises any exception | `503` | `routes_search.py` (catches `Exception`, returns `JSONResponse({"detail": "service unavailable"}, status_code=503)`) | none — meta-lookup failure does not enqueue telemetry |
| Search pipeline timed out (`asyncio.TimeoutError` from `wait_for` around `pipeline.search(...)`) | `504` | `routes_search.py` (`asyncio.wait_for(timeout=_SEARCH_TIMEOUT_SECONDS)`, raises `HTTPException(504)`) | `timeout` (with `status="timeout"`); ERROR log with `event_type="search_timeout"` |
| Search pipeline raised any other exception | `500` | `routes_search.py` (bare `raise` after logging) | `other` (with `status="internal_error"`); ERROR log with `event_type="search_pipeline_failure"` |
