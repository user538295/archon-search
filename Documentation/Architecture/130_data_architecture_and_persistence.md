...
| `models/` | written by `install/licenses.py`, read by `language_detector.py` | fasttext language detector (`lid.176.ftz`) | only if `multilingual=True`; resolved lazily via `paths.get_fasttext_models_dir()` |
| `models/graph/<model>--<revision>/` | `huggingface_hub` (via `GLiNER.from_pretrained`), read by `prose_extraction_backend.py` | pinned GLiNER prose-extraction checkpoint cache; `<model>` is `GRAPH_NER_MODEL_NAME` with its repo-id `/` replaced by `--`, `<revision>` is `GRAPH_NER_MODEL_REVISION` | resolved lazily via `paths.get_graph_models_dir()` — the only accessor, and both constants live in `paths.py` too, so a revision bump moves the directory. No installer or wizard step populates it: it is created on the first `ProseExtractionBackend.load()` and not guaranteed to exist before that. `model_validation.py` never touches this path — `graph_ner_status()` probes engine importability only. A missing or unloadable artifact degrades prose extraction to code-symbol-only instead of failing an ingest |
| `history/sessions/` | `cli/ingest.py` | default `--sessions-dir` for the `ingest` subcommand | resolved lazily via `get_data_dir()` |

Override paths:
- `ARCHON_SEARCH_DATA_DIR` (C9) — relocates the entire layout above (except `archon-search.toml` — see `ARCHON_SEARCH_CONFIG`). The Docker image sets `ARCHON_SEARCH_DATA_DIR=/data`. Read lazily on every call by `paths.get_data_dir()`, `key_manager.get_key_file()`, `jobs.get_jobs_file()`, `paths.get_fasttext_models_dir()`, `paths.get_graph_models_dir()`, `cli/ingest.py`, and `config.load_config()`.
...
The nine durable-write sites that use the helper are (the tenth, the graph model's tree-publishing site, went away with the wizard's model provisioning — the GLiNER checkpoint is now written by `huggingface_hub`, outside this codebase; `_durable_io.fsync_tree` consequently has no production caller today, only `tests/test_durable_io.py`):
...
- **`_archon_graph_{ns}__{col}_nodes`** — one row per unique entity. Key columns: `id` (stable SHA-256-derived entity ID), `entity_type` (e.g. `"concept"`, `"person"`, `"code_symbol"`), `entity_name` (bare display name — for `code_symbol` nodes, this is the unqualified symbol name only, never path-qualified), `chunk_count` (mention count), `name_embedding` (nullable `float64` list — set during synonym enrichment), `pagerank_score` (nullable `float64` — persisted by `GraphStore.write_pagerank_scores` after background `PageRankBuilder` pass; null until first recompute), `source_path` (nullable `utf8` — the source file a `code_symbol` node was extracted from; lets `compute_impact` disambiguate same-named symbols by a caller-supplied `file_path`; the `ensure_graph_tables` S68 migration only *adds* this nullable column to pre-S68 tables — it does not populate existing rows, so it stays `NULL` on legacy rows until the file is re-ingested; `compute_impact` falls back to highest-pagerank resolution when every candidate's `source_path` is still `NULL`). Written by `GraphStore.write_graph` during ingest.
- **`_archon_graph_{ns}__{col}_edges`** — one row per relationship. Key columns: `id` (stable SHA-256-derived edge ID), `source_node_id`, `target_node_id`, `relationship_type` (string from `RelationshipType` enum — values: `"uses"`, `"implements"`, `"depends_on"`, `"related_to"`, `"synonym_of"`, `"calls"`, `"imports"`, `"defines"`, `"inherits"` — the three the prose engine makes primary are `"uses"`, `"implements"` and `"depends_on"`: directed, non-normalised edges written straight from the GLiNER checkpoint's own relation output at ingest, with no LLM provider and no network involved), `extraction_method` (nullable string — `"manual"` for alias-file synonyms, `"embedding"` for ANN-detected synonyms, `"extracted"` for AST-proven def/ref edges, `"inferred"` for cross-file best-guess matches; **every prose-graph edge leaves it `None`** — an earlier revision of this line claimed `"ner"`, a value no code has ever written, exactly the vocabulary-by-example risk flagged in `Completed/e2g-k1-contract-confirmation.md`. That covers both prose shapes: the undirected, `sorted()`-normalised `"related_to"` co-occurrence edge, of which the prose path is the only producer, and the directed typed edges the engine asserts. So the two prose shapes are told apart by `relationship_type`, not by `extraction_method`; and a typed edge is told apart from a def/ref edge of the *same* `relationship_type` by `extraction_method is None` (def/ref writes `"extracted"`/`"inferred"`). One consequence worth knowing: the engine's relation vocabulary also includes `"calls"`/`"imports"`/`"defines"`/`"inherits"`, and `_edge_is_defref` classifies **any** edge of those four types as def/ref tier regardless of `extraction_method`, so a prose-asserted edge of one of them is GC-exempt — `_MENTION_DERIVED_RELATIONSHIP_TYPES`, the set the unsupported-edge sweep works over, is exactly `{related_to, uses, implements, depends_on}`), `source_doc_id` (last writer wins — edge IDs carry no `doc_id`, so two documents asserting the same pair share one row; used by `delete_graph_by_doc`, **not** by orphan GC, which works from the mentions table). Written by `GraphStore.write_graph` during ingest. Tag-collision precedence: `"extracted"` always wins over `"inferred"` and is never downgraded (Q11, E2g).
...
**`_archon_graph_{ns}__{col}_communities`** (`GraphStore.ensure_communities_table()`):
...
1. **SQL WHERE clause** (LanceDB-side, `store_filters.build_where`): handles `file_type`, `source_path_prefix`, `indexed_after`, `indexed_before`. **E2a**: also handles exact `scope_filter` values via `(scopes IS NULL OR list_has(scopes, '<value>'))` — the `scopes IS NULL` arm ensures unscoped chunks always pass through. These are expressed as SQL predicates pushed into the LanceDB query. For how values are quoted into those predicates and the CI guard that enforces it, see [`150_security_and_privacy_architecture.md` → "SQL boundary defense-in-depth (A5b)"](./150_security_and_privacy_architecture.md#sql-boundary-defense-in-depth-a5b).
