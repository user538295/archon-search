...
| `models/spacy/en_core_web_sm-<ver>/` | written by `install/extras.py`, read by `graph_extractor.py` | spaCy NER model directory, unpacked from the pinned spacy-models wheel | only when the wizard provisioned it; resolved lazily as `paths.get_models_dir() / "spacy"` (there is no longer a dedicated `get_spacy_models_dir()` accessor — call sites construct the path inline). `model_validation.py` never touches this path directly — it resolves the model through `graph_extractor.find_spacy_model()`. The runtime never writes here — a missing model degrades prose NER instead of failing ingest (2026-08-19-030) |
| `models/graph/<model>-<revision>/` | `huggingface_hub` (via `GLiNER.from_pretrained`) | pinned GLiNER checkpoint cache (`GRAPH_NER_MODEL_NAME`/`GRAPH_NER_MODEL_REVISION`) | resolved lazily via `paths.get_graph_models_dir()`; not guaranteed to exist, populated on first use |
...
- **`_archon_graph_{ns}__{col}_edges`** — one row per relationship. Key columns: `id` (stable SHA-256-derived edge ID), `source_node_id`, `target_node_id`, `relationship_type` (string from `RelationshipType` enum — values: `"uses"`, `"implements"`, `"depends_on"`, `"related_to"`, `"synonym_of"`, `"calls"`, `"imports"`, `"defines"`, `"inherits"`), `extraction_method` (nullable string — `"manual"` for alias-file synonyms, `"embedding"` for ANN-detected synonyms, `"extracted"` for AST-proven def/ref edges, `"inferred"` for cross-file best-guess matches; **spaCy co-occurrence edges leave it `None`** — an earlier revision of this line claimed `"ner"`, a value no code has ever written, exactly the vocabulary-by-example risk flagged in `Completed/e2g-k1-contract-confirmation.md`; identify them by `relationship_type == "related_to"`, which the NER path is the only producer of; **LLM-typed edges** (`"uses"`/`"implements"`/`"depends_on"`, `graph_extractor.py:758-763`) **also leave it `None`** — they are distinguished from spaCy edges by `relationship_type != "related_to"`, not by `extraction_method`), `source_doc_id` (last writer wins — edge IDs carry no `doc_id`, so two documents asserting the same pair share one row; used by `delete_graph_by_doc`, **not** by orphan GC, which works from the mentions table). Written by `GraphStore.write_graph` during ingest. Tag-collision precedence: `"extracted"` always wins over `"inferred"` and is never downgraded (Q11, E2g).
