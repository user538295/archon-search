...
| `GET /ready`             | None        | `{"ready": bool, "checks": {"storage": "ok"\|"fail", "models": "pending"\|"ok"\|"warn"\|"fail"}}`. Readiness probe; returns HTTP 200 when storage is connected and eager model warm-up is not still pending, HTTP 503 otherwise. **D6** — `checks.models` reflects the background model-validation result, which does **not** gate `ready` or the HTTP status. The exception is eager warm-up: with `[database].eager_load_embedders = true`, `checks.models` reads `"pending"` and the endpoint answers 503 until the warm-up task finishes. | `server/routes_ready.py`                |
...
The `start-period=600s` gives the storage layer (and the first-start runtime-extras install, plus any eager warm-up) time to finish before failures count against the `retries` budget; once `SearchStore.ping()` returns OK, `docker ps` reports `(healthy)`. **The `/ready` `ready: bool` (and thus this HEALTHCHECK) is not gated on model-validation state** — D6 added a `checks.models` field that *reports* background model-validation state, but it never affects `ready` or the HTTP status. It **is** gated on eager warm-up: with `[database].eager_load_embedders = true` the container reports unhealthy until the warm-up task completes, which is the supported way to make the orchestrator wait for a search-ready server. Without that flag the first `/search` after a cold container start still pays a multi-second model-load tax.
...
2. `curl http://127.0.0.1:8765/ready` — confirms the storage layer is connected and the service can accept queries. HTTP 503 with `{"ready": false, "checks": {"storage": "fail", "models": ...}}` means the LanceDB store is not reachable; restart the service or check `~/.archon-search/logs/archon-search.log`. A 503 with `"storage": "ok"` and `"models": "pending"` is not a fault — eager warm-up is still loading models; wait and retry. This step requires no API key.
