...
| `GET /ready`             | None        | `{"ready": bool, "checks": {"storage": "ok"\|"fail", "models": "pending"\|"ok"\|"warn"\|"fail", "sync": "pending"\|"ok"\|"warn"\|"fail"}}`. Readiness probe; returns HTTP 200 when storage is connected, eager model warm-up is not still pending, **and** the lifespan's startup collection sync is not still running; HTTP 503 otherwise. **D6** — `checks.models` reflects the background model-validation result, which does **not** gate `ready` or the HTTP status. The exception is eager warm-up: with `[database].eager_load_embedders = true`, `checks.models` reads `"pending"` and the endpoint answers 503 until the warm-up task finishes. The startup-sync gate is independent of that flag: `checks.sync` reads `"pending"` (and the endpoint answers 503) while the startup sync task is still running, else `"fail"` if the sync raised or completed with per-collection errors in `SyncResult.errors`, else `"warn"` if the crash-loop guard suppressed the sync (`sync_result == "suppressed"`), else `"ok"` — like `models`, `sync: "fail"`/`sync: "warn"` are informational only and do NOT gate `ready` (`/ready` answers 200 with either). | `server/routes_ready.py`                |
| `GET /status`            | Bearer      | Top-level `running` (bool, always `true` when this handler responds), `pid`, `version`, `warmup_result` (`"pending"\|"done"\|"failed"\|null`; `null` when `eager_load_embedders = false`, i.e. no warm-up was ever run), `sync_result` (`"pending"\|"done"\|"failed"\|"suppressed"\|null`; `null` when no collections are configured, `"suppressed"` when the crash-loop guard refused to start the startup sync), a per-collection list with `name`, `path`, `doc_count`, `chunk_count`, `status`, `watching`, `processed_files`, `total_files`, `eta_seconds`, `error`, `error_count` (filtered to caller's namespace), and a `readiness` sub-object (`storage_connected`, `embedder_warm`, `reranker_warm`, `jobs`, `collections_indexing`, `collections_failed`, `watcher`). `path`, `doc_count`, and `chunk_count` are currently always `""` / `0` placeholders. | `server/routes_status.py`               |
...
`SearchInstaller.configure_providers` writes the matching ONNX provider into `[database].providers` by default; a `section=` / `providers=` pair targets any other section with an explicit list (including the empty list, the explicit CPU write). As of C0, `archon-search install` calls `SearchInstaller.run()`, which invokes `detect_gpu_type()` and `configure_providers()` during the install flow — GPU detection runs automatically and the correct ONNX provider is written to `archon-search.toml` without manual intervention. Operators can override by editing `[database].providers` after installation.

`[graph].providers` is settled separately, after the Step 14 pre-warm, by a two-stage device probe (`install/device_probe.py`): stage 1 (`probe_device_availability`) is a free pre-download check, stage 2 (`probe_device_validation`) reports the device the pre-warm actually placed the model on. Only when both stages validate a non-CPU device is the operator offered the accelerator, with the accelerator as the default. Either stage failing shows no prompt: CPU (`[graph].providers = []`) is written silently and the reason is logged at WARNING, never surfaced to the operator. Pre-warm not having run at all (`--skip-preload`) is the third case — no prompt and no write, because an unset list already reads as CPU and there is no evidence to justify overwriting a list the operator set by hand. Nothing is settled when graph extraction is not installed, or when its extra failed to install. `--graph-providers` supplies the candidate list for non-interactive installs (which otherwise take no accelerator) and is itself overridden by `--disable-gpu`; the same validation gate still decides whether the accelerator or CPU is written.
...
The `start-period=600s` gives the storage layer (and the first-start runtime-extras install, plus any eager warm-up and startup sync) time to finish before failures count against the `retries` budget; `docker ps` reports `(healthy)` only once `SearchStore.ping()` returns OK **and** (when applicable) eager warm-up and the startup sync have both completed — see below. **The `/ready` `ready: bool` (and thus this HEALTHCHECK) is not gated on model-validation state** — D6 added a `checks.models` field that *reports* background model-validation state, but it never affects `ready` or the HTTP status. It **is** gated on eager warm-up: with `[database].eager_load_embedders = true` the container reports unhealthy until the warm-up task completes, which is the supported way to make the orchestrator wait for a search-ready server. Without that flag the first `/search` after a cold container start still pays a multi-second model-load tax.

`/ready` is **also** gated on the lifespan's startup collection sync — unconditionally, regardless of `eager_load_embedders`. The sync itself is bounded: `_STARTUP_SYNC_TIMEOUT_SECONDS = 1800.0` (`app.py`) wraps it in `asyncio.timeout(...)`, so a wedged or oversized sync is force-marked `checks.sync: "fail"` at 1800s and `/ready` recovers to `200` — it does not gate `ready` (see above), so this is not an unbounded hang. That 1800s bound is well above the default `start-period=600s` + `interval=15s --retries=3` (~645s of grace before `retries` are exhausted), so on a first boot against a large corpus `docker ps` can still report the container unhealthy well before the sync's own ceiling fires. Under a restart policy or an orchestrator's `depends_on: service_healthy`/liveness-probe-triggered restart (plain `docker run`/`docker-compose` does not restart on unhealthy by itself), this can restart the container somewhere in that ~645s–1800s window, discarding the in-flight sync attempt. The bound guarantees the sync eventually terminates and stops gating `ready` — it does not guarantee the container stays `(healthy)` through a single attempt — but because the sync is incremental (already-indexed files are skipped on the next run), a restarted attempt still makes net progress, so successive boots converge on a fully-synced index even under repeated restarts; avoiding the restarts (and the wasted re-scanning) entirely is still preferable. Operators seeding a large corpus on first boot should either raise `--start-period` (or the compose/Kubernetes equivalent) past the expected sync duration, or pre-warm the volume by running an initial sync out-of-band (e.g. `docker run` once with `--health-cmd` disabled, or `archon-search sync --wait` before wiring the container into a restart-triggering orchestrator) so subsequent starts see an already-synced index. `checks.sync: "pending"` on `/ready` (`docker exec <container> curl -s localhost:8765/ready`) confirms this cause directly (`"fail"` once the 1800s bound has been hit); `docker logs` also shows `"startup sync"` log lines to distinguish "still syncing" from a hung or crashed sync. This entire risk is conditional on at least one collection being configured (`pinned_collections` or `[collections]` non-empty, `app.py`) **and** on the crash-loop guard not firing — when the job store reports `crashed_ingest_on_load` (an ingest-family job was rewritten to `FAILED / "process_restart"`, i.e. the previous process died mid-ingest — including inside a previous startup sync, which runs under a `SyncJob` of its own so it leaves that marker) **or** the sticky suppression sentinel file (`archon-search-sync-suppressed.json`, `sync_suppression.py`, under the data dir) already exists from an earlier boot, the task is deliberately not spawned either, and `GET /status` reports `sync_result: "suppressed"` (`/ready`: `checks.sync: "warn"`, still 200). Suppression is **sticky across restarts**: it persists on every boot after the crash — even once the crashed job row has gone terminal and `crashed_ingest_on_load` would no longer fire on its own — until an operator runs `POST /sync`, whose clean completion (no exception, no per-collection errors) deletes the sentinel and moves `sync_result` from `"suppressed"` **or** `"failed"` to `"done"`: with an empty config the startup-sync task is never spawned, so `checks.sync` is always `"ok"` and none of the above applies — this is the state of the bare `docker run` quickstart in `UserManual/140_running_with_docker.md`.
...
`archon-search stop` blocks until the OS supervisor reports the service actually stopped, so that once it returns a subsequent `archon-search status` reads `stopped` and `GET /health` is unreachable (S04). `launchctl unload` / `systemctl --user stop` return before the process has fully exited and released its listening socket, so `stop()` polls `status()` (up to ~10 s) after issuing the stop command. On the normal path the CLI prints `archon-search stopped`. If the service is still up at the deadline, `stop()` returns non-zero and logs a WARNING (`archon-search did not report stopped within 10s…`); because the short-lived `stop` CLI process configures no file logging (that is the daemon's job), this WARNING surfaces on stderr, not in `~/.archon-search/logs/archon-search.log`. The CLI, instead of the clean success line, then prints `Warning: archon-search did not confirm stopped within the timeout; it may still be running. Check archon-search status.` to stderr and still exits 0. `uninstall` (`unregister`) deliberately does not wait: it removes the plist/unit and makes no `/health`-down guarantee.
...
2. `curl http://127.0.0.1:8765/ready` — confirms the storage layer is connected and the service can accept queries. HTTP 503 with `{"ready": false, "checks": {"storage": "fail", "models": ..., "sync": ...}}` means the LanceDB store is not reachable; restart the service or check `~/.archon-search/logs/archon-search.log`. A 503 with `"storage": "ok"` and `"models": "pending"` is not a fault — eager warm-up is still loading models; wait and retry. A 503 with `"storage": "ok"`, `"models": "ok"`, and `"sync": "pending"` is also not a fault — the server's own startup collection sync is still running; check the log for `"startup sync"` lines to confirm it is progressing before assuming a hang. This step requires no API key.
