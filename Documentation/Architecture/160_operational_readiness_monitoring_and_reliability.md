...
| `GET /indexing-state`    | Bearer      | Machine-readable raw indexing state: per-collection `status`, file counters, timestamps, error, `error_count`. Empty object when no state file exists. `IndexingStateStore` is now thread-safe (CON-3 closed in A6); concurrent ingest across collections no longer races on this file. | `server/routes_state.py`                |
