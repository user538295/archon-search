...
| API-3 | MCP-mirrors-REST aspiration is partial. MCP exposes 13 tools (as of D1/D2: `search`, `search_with_context`, `explain`, `ingest_file`, `ingest_directory`, `list_collections`, `get_collections_meta`, `get_collection_meta`, `list_documents`, `delete_document`, `update_collection`, `export_collection`, `import_collection`); REST also surfaces `state`, `status`, `route`, `jobs` (list + resume), `telemetry`, and collection `reindex`/`add`/`remove`. The `GET /jobs` list and `POST /jobs/{id}/resume` endpoints have no MCP mirror yet. The naming gap (`search_*` prefix in CLAUDE.md vs. bare names in code) compounds the drift. | API contract | Low | New REST endpoint added; downstream MCP client requests parity. | `archon_search/server/mcp.py`, `archon_search/server/app.py` (9 routers), [`520_api_design_and_contracts.md`](./520_api_design_and_contracts.md) |
