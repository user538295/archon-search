...
- Parallelism: the default run uses `pytest-xdist` with `-n auto --dist=loadfile`. `--dist=loadfile` keeps all tests within a single file on one worker, which is required to preserve `module`-scoped fixture semantics (e.g. `connected_store` in `tests/conftest.py` avoids spawning a fresh LanceDB Tokio runtime per test; `col_name` mints a UUID-suffixed name to prevent cross-test pollution).
