...
- Test infra: `tests/conftest.py` installs ML stubs at module import time via `_search_stubs.install_stubs()`. It also injects `ARCHON_SEARCH_API_KEY = "0" * 64` so `create_app()` always sees a known key. The `connected_store` fixture is module-scoped to avoid spawning a fresh LanceDB Tokio runtime per test; the `col_name` fixture mints a UUID-suffixed name to prevent cross-test pollution.
