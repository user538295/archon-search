...
| Module | Purpose | Key public symbols |
|---|---|---|
| `archon_search/router.py` | Fetch collection metadata over JSON-RPC, score each centroid against the query embedding, apply a confidence gate, build a three-tier shortlist for the decomposer. Accepts `initial_metadata` for constructor-time injection and exposes `invalidate()` to clear the cached metadata for long-lived router instances. | `MultiCollectionRouter` (incl. `invalidate`, `initial_metadata` ctor param) |
