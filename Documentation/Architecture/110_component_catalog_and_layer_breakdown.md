...
| `archon_search/server/routes_status.py` | `GET /status`. **D6** — reads `app.state.model_validation` (via `getattr` guard) and includes `model_validation: ModelValidationStatus \| None` in the response (`None` while pending). | `router` |
