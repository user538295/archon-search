...
| `extraction_model` | `null` | Bare model name for the configured `provider` (e.g. `"claude-haiku-4-5-20251001"`, an Ollama tag, or a llama.cpp model name — the wizard writes the name shown by `llama cli -cl`) — never a `"provider:model"` string. Logs a WARNING once at build time if `provider` is set but this is empty. |
