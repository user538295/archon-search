...
1. **Start at `/health` and `/ready`, then `/status`.** They are the cheapest probes. `GET /health` tells you the process is alive; `GET /ready` tells you whether storage is connected, any eager model warm-up has finished, and the startup collection sync has finished (the model-*validation* result it also reports is informational); `GET /status` tells you what the indexer, jobs, and optional features are doing. All three are auth-exempt for `/health` and `/ready`; `/status` needs a Bearer token.
...
`GET /ready` returns **503** in three cases: the **storage** check fails (`store.ping()` — LanceDB unreachable / not initialised), an eager model warm-up is still running, or the server's own startup collection sync (run automatically at boot over all configured collections) has not finished yet. A `warn`/`fail` model-validation result is **informational** and never flips `ready` to false. In the JSON body:

- `checks.storage` — `ok` / `fail`. `fail` → HTTP 503; fix the datastore (disk, permissions, `db_path`).
- `checks.models` — `ok` / `warn` / `fail` / `pending`. `pending` means either the background model probe has not finished yet, or (with `[database].eager_load_embedders = true`) the eager warm-up is still loading models — the latter also returns HTTP 503, and is normal right after start; just wait and retry. `warn`/`fail` do not block readiness but flag a model/provider issue — see below. `fail` has two distinct causes, checked in this order: a **terminally failed eager warm-up** (the warm-up task raised, or exceeded its 600s ceiling — check the log for `"eager model warm-up failed: ..."`; search still works via lazy-load fallback, just paying a first-query cost), or the separate background `model_validation` probe reporting the configured embedding/reranker model could not load at all (this one *does* mean search will fail until fixed).
- `checks.sync` — `ok` / `pending` / `warn` / `fail`. `warn` means the automatic startup sync was **suppressed**: the previous run died mid-ingest (OOM, `kill -9`), so the crash-loop guard refused to re-enter the same workload. This suppression is **sticky** — it survives a process restart (a sentinel file under the data dir keeps it armed) and keeps firing on every subsequent boot, not just the one right after the crash. `/ready` still answers 200 (`ready: true`) and `GET /status` reports `sync_result: "suppressed"`; the index stays as it was until you fix the cause and run `POST /sync` (or `archon-search sync`), which clears both back to `ok` / `"done"`. `pending` means the server's own startup collection sync (run automatically at boot over all configured collections) has not finished yet — normal on a first boot against a large corpus. Check the log for `"startup sync"` lines to confirm it is progressing rather than stuck; `POST /sync` (and `archon-search sync`) also returns `409` for the same window. `fail` means the startup sync either raised an exception or completed but recorded per-collection errors (missing path, ingest failure) — check the log for a `"startup sync failed"` or `"startup sync completed with"` WARNING, then investigate and retry with `POST /sync` (or `archon-search sync`). Unlike `pending`, `fail` does NOT make `/ready` answer 503: the server still reports `ready: true` so a corrupted collection cannot wedge the pod's readiness forever.
