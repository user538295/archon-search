...
curl http://127.0.0.1:8765/ready     # readiness — 200 once storage is connected, any eager warm-up finished, and the startup sync finished, 503 before
curl -H "Authorization: Bearer $ARCHON_SEARCH_API_KEY" http://127.0.0.1:8765/status
```

The image declares a `HEALTHCHECK` that polls `/ready` every 15s after a 600s start-period. The generous start-period accommodates the network-bound first-start extras install (see "Entrypoint and runtime extras installation" above); subsequent starts are instant (stamp matches) but the start-period is fixed at image level. `docker ps` shows `(healthy)` once the storage layer is up, any eager warm-up has finished, and the server's own startup collection sync has finished.

**First-boot corpus sync can exceed the 600s start-period.** `/ready` is gated on that startup sync unconditionally (not just with `eager_load_embedders = true`). The sync itself is bounded to 1800s (`_STARTUP_SYNC_TIMEOUT_SECONDS`): a wedged or oversized sync is force-marked `checks.sync: "fail"` at that point and `/ready` recovers to `200`, so it cannot hang forever. That bound is well above the default `start-period=600s` + `interval=15s --retries=3` (~645s of grace), so seeding a large corpus on first boot can still make `docker ps` report the container **unhealthy** well before the sync finishes — and under a restart policy or `depends_on: service_healthy` (plain `docker run`/`docker-compose` does not restart on unhealthy by itself), an orchestrator that restarts unhealthy containers may restart it somewhere in that ~645s–1800s window, discarding the in-flight attempt. Because the sync is incremental — already-indexed files are skipped — a restarted attempt still makes progress, so successive boots converge even under repeated restarts, but avoiding the restarts entirely is still preferable. If your first-boot corpus is large: raise `--start-period` (or the compose/Kubernetes equivalent) past the expected sync duration, or pre-seed the mounted volume with an already-synced index before wiring the container into anything that restarts on unhealthy. `curl /ready | jq .checks.sync` reads `"pending"` while this is the cause, or `"fail"` if the 1800s bound was hit; `docker logs` also shows `"startup sync"` log lines while it runs. This restart-loop risk only applies once at least one collection is configured (`[collections]` or `pinned_collections` non-empty) — the startup sync task is never created with an empty config, so the bare `docker run` quickstart above never hits `checks.sync: "pending"` and always sees `"ok"`. The task is likewise not created when the crash-loop guard fires (the previous container died mid-ingest, so loading the job store rewrote an ingest-family job to `FAILED / "process_restart"`) **or** a sticky suppression sentinel file (`archon-search-sync-suppressed.json`, under the data dir volume) already exists from an earlier container start: `checks.sync` reads `"warn"`, `/ready` answers `200` immediately, `docker ps` reports `(healthy)`, and `GET /status` reports `sync_result: "suppressed"`. Suppression **survives container restarts** — it keeps firing on every restart, not just the one immediately after the crash, because the sentinel lives on the mounted data volume — the index stays as the crash left it until an operator runs `POST /sync` (or `archon-search sync`), whose clean completion deletes the sentinel and moves `sync_result` to `"done"` and `checks.sync` back to `"ok"`.
...
| `ARCHON_SEARCH_DATA_DIR` | `/data` (baked into image) | Root of every runtime path: `db_path` (`/data/search`), `log_file` (`/data/logs/archon-search.log`), `telemetry.log_dir` (`/data/search-logs`), key file (`/data/.search.env`), managed key store (`/data/keys.json`), jobs file (`/data/archon-search-jobs.json`), sync-suppression sentinel (`/data/archon-search-sync-suppressed.json`, present only while the startup-sync crash-loop guard is suppressed), fasttext models (`/data/models`), ingest history (`/data/history/sessions`). One env var relocates the entire runtime tree — do not change it unless you also remount the volume. |
...
├── archon-search-sync-suppressed.json  # present only while the startup-sync crash-loop guard is suppressed
...
- **`GET /ready` also gates on the startup collection sync, unconditionally** — the container stays unhealthy (503, `checks.sync: "pending"`) until the lifespan's own startup `collection_sync.sync()` over all configured collections finishes (see "First-boot corpus sync can exceed the 600s start-period" under "`docker run` quickstart" above for the restart-loop risk on a large first-boot corpus). `POST /sync` (and `archon-search sync`) returns `409` for the same window. The one exception is the crash-loop guard: when the previous run died mid-ingest, or a sticky suppression sentinel from an earlier boot is still present on the data volume, no startup sync task is created at all, so the container reports `(healthy)` immediately with `checks.sync: "warn"` and `sync_result: "suppressed"` — informational, never a 503. Suppression persists across container restarts until a clean `POST /sync` clears the sentinel.
