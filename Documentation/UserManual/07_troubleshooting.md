...
1. **Check `GET /status`** — if `[hyde] enabled = true` or `[rag_fusion] enabled = true` in your config, the response includes `hyde.key_available` or `rag_fusion.key_available`. A `false` value means the provider's required API key is not set. For `provider = "anthropic"` (default): `ANTHROPIC_API_KEY`; for `provider = "openai"`: `OPENAI_API_KEY`; for `provider = "ollama"`: `key_available` is always `true` — if you see `false` with Ollama, check your `[hyde].provider` config value.
2. **For the managed service**: the required key must be in `~/.archon-search/.secrets.env` (one line: `ANTHROPIC_API_KEY=sk-...` for Anthropic, or `OPENAI_API_KEY=sk-...` for OpenAI). Not required for Ollama. The wizard creates this file automatically (mode 600, empty) when HyDE or RAG Fusion is enabled — if you skipped the wizard, create it manually. The managed service sources this file at start time via the wrapper script (macOS) or `EnvironmentFile=` (Linux systemd). After editing the file, restart the service (`archon-search stop && archon-search start`).
3. **For `archon-search serve` (container mode)**: pass the key as an environment variable: `-e ANTHROPIC_API_KEY=sk-...` (Anthropic) or `-e OPENAI_API_KEY=sk-...` (OpenAI). Not required for Ollama.
4. **Run `archon-search status`** — it warns on stderr when HyDE or RAG Fusion is enabled with a non-Ollama provider but the required API key is absent.
5. If the key is set but you still see `expansion_warning`, the LLM provider API call may be timing out. Raise `timeout_seconds` in the `[hyde]` or `[rag_fusion]` TOML section (default is `10.0` seconds). For `provider = "ollama"`: if expansion fails, check that your Ollama server is running at `ollama_base_url` (default `http://localhost:11434`).
