...
1. **Check `GET /status`** — if `[hyde] enabled = true` or `[rag_fusion] enabled = true` in your config, the response includes `hyde.key_available` or `rag_fusion.key_available`. A `false` value means the provider's required API key is not set. For `provider = "anthropic"` (default): `ANTHROPIC_API_KEY`; for `provider = "openai"`: `OPENAI_API_KEY`; for `provider = "ollama"` and `provider = "claude_cli"`: `key_available` is always `true` (no API key). For `claude_cli`, `key_available: true` does **not** confirm the `claude` command is installed or logged in — a missing/logged-out CLI surfaces its error on first use and expansion falls back silently (run `claude -p "hi"` yourself to confirm it works).
