...
2. **The server must be running.** Because write commands proxy to REST, they require `archon-search serve` (or `start`). On connection refused they print `archon-search serve is not running. Start it first with: archon-search serve` (`cli/_helpers.py:_SERVER_NOT_RUNNING_MSG`). Inside the `--wait` poll loop the message is chosen by `_server_connect_fail_msg()`: if the service process is alive but the port is not yet bound — the startup window while models are still loading — it prints `archon-search is starting up. Please wait for it to finish loading models, then retry.` instead.
