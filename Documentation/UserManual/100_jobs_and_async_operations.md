...
2. **The server must be running.** Because write commands proxy to REST, they require `archon-search serve` (or `start`). On connection refused the message is chosen by `_server_connect_fail_msg(base_url)` (`cli/_helpers.py`), which probes `GET /ready`: if that answers `503` with `checks.models: "pending"` and `checks.storage: "ok"` — the startup window while models are still loading — it prints `archon-search is starting up. Please wait for it to finish loading models, then retry.`; otherwise it prints `archon-search serve is not running. Start it first with: archon-search serve` (`_SERVER_NOT_RUNNING_MSG`). Because the probe hits the server directly, the "starting up" hint works for a foreground `archon-search serve` too, not just a managed service.
...
Output columns: `ID` (first 8 chars), `TYPE`, `COLLECTION`, `STATUS`, `STARTED`, `ELAPSED`. Whenever at least one job is shown, it prints `Showing N of M jobs.` below the table (where `N` is the row count and `M` the total matching jobs across all pages). When the listing was truncated (`N < M`), the line reads `Showing N of M jobs — use --limit to see more (max: 200).` instead. When no jobs match, it prints `No jobs found.` with no table.
