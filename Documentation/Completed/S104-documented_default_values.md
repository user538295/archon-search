...
**ID**: S104-documented_default_values
**Scenario**: S104
**Severity**: medium
**Version**: archon-search, version 26.8.1916

### What happened
AssertionError: [logging].level does not show its documented default (no match for 'level\\s*=\\s*"INFO"'):
[collections]
...
### Evidence
```
E   AssertionError: [logging].level does not show its documented default (no match for 'level\\s*=\\s*"INFO"'):
E     [collections]
...
E     
E   assert None
E    +  where None = <function search at 0x109c4b320>('level\\s*=\\s*"INFO"', '[collections]
...
')
E    +    where <function search at 0x109c4b320> = re.search
```
