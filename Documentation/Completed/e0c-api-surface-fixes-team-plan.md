...
**Done when**
- [x] `sample_chunk_texts` returns shuffled results; `_MAX_SAMPLE_CHUNKS = 100` — S15, S16
- [x] `POST /search` fanout validated from `config.max_fanout`, not the removed constant — S8, S9
- [x] `POST /search` `top_k` validated from `config.top_k_max` (default 100) — S11, S12, S14
- [x] `GET /status` returns `search: { max_fanout, top_k_max }` — S10, S13
- [x] `GET /collections/{name}/documents` REST endpoint with cursor pagination — S1–S6
- [x] MCP `list_documents` accepts optional `cursor` additively — S7
