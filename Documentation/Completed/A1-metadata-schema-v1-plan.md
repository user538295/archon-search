...
- **Acceptance criteria** (must all pass):
  ...
  - [x] *During an active reindex of collection A, `POST /ingest` to collection A returns HTTP 503 with `Retry-After: 30` and JSON body `{"error": "store_busy", ...}`. Ingest to a **different** collection succeeds normally. MCP `ingest_file` and `ingest_directory` surface `StoreBusyError` as `McpErrorResponse(code="store_busy")`.* (Delivered by **A5c** — `Documentation/Backlog/A5-ingest-hardening-plan.md` Task 2c.3. Store-layer half pinned by `tests/test_store_lock.py` from A1 Task 6.1; synchronous REST 503 surface and MCP surface completed in A5c.)
