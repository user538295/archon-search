...
**Done when**
- [x] `--wait --timeout N` accepted by `maintenance run`, `export`, and `backup --now`; exits 0 on timeout with recovery message — S21, S22, S23, S24, S25
- [x] `archon-search maintenance run --wait` exits 2 when job FAILED — S23
- [x] `archon-search status` warns when HyDE/RAG Fusion enabled but key absent — S7, S8
- [x] `archon-search status` shows FAILED_EXPIRED ingest count — S15
- [x] `archon-search ingest` prints ACL sidecar warnings to stderr — S19
...
**Done when**
- [x] `SearchPipelineResult.rag_fusion_warning` populated on RAG Fusion timeout; route assembles `expansion_warning` from HyDE detection + `rag_fusion_warning` — S3, S4, S4b
- [x] `SearchResponse` carries `expansion_used` and `expansion_warning` — S1, S2, S3, S4, S4b, S5
- [x] `GET /status` carries `hyde.key_available`, `rag_fusion.key_available`, `failed_expired_ingest_count` — S6, S7, S13, S14
- [x] `FAILED_EXPIRED` in `JobStatus`; maintenance loop transitions aged-out and retry-exhausted jobs — S11, S11b, S12, S12b
- [x] `GET /telemetry/stats` carries `truncated_count` — S16, S17
- [x] `IngestResult.warnings` populated from ACL sidecar check — S18, S19, S20
- [x] Linux service unit gains `EnvironmentFile=-`; macOS service generates wrapper script — S9, S10
- [x] OpenAPI snapshot regenerated and CI passing
