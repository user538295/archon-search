...
- Recognised environment overrides: `ARCHON_SEARCH_CONFIG` (config path), `ARCHON_SEARCH_API_KEY` (API key), `ARCHON_SEARCH_KEY_FILE` (key file path), plus `ARCHON_SEARCH_HOST`, `ARCHON_SEARCH_PORT`, and `ARCHON_SEARCH_DATA_DIR`. The registered plist/unit bake in `ARCHON_SEARCH_CONFIG=~/.archon-search/archon-search.toml`; the Docker image bakes in `ARCHON_SEARCH_DATA_DIR=/data`, `ARCHON_SEARCH_CONTAINER=1`, and `FASTEMBED_CACHE_PATH=/data/fastembed-cache`.
