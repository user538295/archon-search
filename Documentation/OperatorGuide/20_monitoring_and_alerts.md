...
2. **Liveness and readiness are distinct.** `GET /health` proves only that the event loop answers (liveness). `GET /ready` proves three things: the storage connection is serviceable, any eager model warm-up has finished, and the lifespan's startup collection sync has finished (readiness). Neither forces a model load. See "Liveness vs readiness" below.
...
| `GET /ready` | None | Three gating checks: storage connectivity, eager model warm-up (when `[database].eager_load_embedders = true`), and the lifespan's startup collection sync. `200` `{ready:true, checks:{storage:"ok", models:..., sync:"ok"}}` when all three clear; `503` `{ready:false, ...}` when any fails or is still pending (`routes_ready.py`). `checks.models` is informational only and never changes the status code by itself — a pending eager warm-up is the exception that does. `checks.sync` reads `"pending"` while the startup sync is running, else `"fail"` if the sync raised or completed with per-collection errors, else `"warn"` if the crash-loop guard suppressed it (the previous process died mid-ingest, or a sticky sentinel file from an earlier boot already marked it suppressed — suppression persists across restarts until cleared; `GET /status` shows `sync_result: "suppressed"` and a successful `POST /sync` clears both), else `"ok"`; `"pending"` gates the status code, but `"fail"` and `"warn"` do NOT — both still answer `200`, since neither a corrupted collection nor a stale index must wedge readiness forever (check the log for a WARNING to confirm which). | Anything beyond the coarse `storage`/`models`/`sync` check flags — no names, paths, counts, or queue depth (by design). |
| `GET /status` | Bearer | Richest endpoint. Service fields `running`, `pid`, `version`, `sync_result` (`"pending"`/`"done"`/`"failed"`/`"suppressed"`/`null` — `"suppressed"` means the crash-loop guard skipped the startup sync because the previous process died mid-ingest); per-collection `path`, `doc_count`, `chunk_count`, `status`, `processed_files`, `total_files`, `eta_seconds`, `error`, `error_count`, `watching`; plus the `readiness`, `model_validation`, `backup`, `maintenance`, `telemetry`, `graph`, and other sub-objects (`routes_status.py`); namespace-filtered. | Search latency (see `/telemetry/*`), recent per-query failures. |
...
### `GET /ready` — readiness (storage + eager warm-up + startup sync; everything else informational)

`/ready` runs **three gating checks**: a storage ping (`SearchStore.ping()`, TTL-cached, timeout-guarded); only when `[database].eager_load_embedders = true`, whether eager warm-up is still pending; and, unconditionally, whether the lifespan's backgrounded startup collection sync (`app.state._startup_sync_task`) is still running. It returns `200 {ready:true}` once the store is connected, no warm-up is outstanding, and no startup sync is in progress, and `503 {ready:false}` when the storage ping fails or times out, while the warm-up task is still building the ONNX models, or while the startup sync is rebuilding the index (`routes_ready.py`). The `503` body is the `ReadinessResponse` model itself, **not** the `{"detail": ...}` error envelope — a 503 here is an expected "not ready yet" signal, not a pipeline error. Distinguish the causes from the body: `checks.storage: "fail"` is a datastore fault; `checks.storage: "ok"` with `checks.models: "pending"` is warm-up in progress; `checks.sync: "pending"` means the startup sync is still running; `checks.sync: "fail"` means the sync either raised or completed with per-collection errors, but unlike the other two causes it does NOT gate the status code — `/ready` still answers `200` in that case.
...
- While the startup sync gate is active, `POST /sync` (and `archon-search sync`) also answers `409` — both share `app.state.sync_lock`.
...
`provider_warnings` also carries **graph prose-extraction engine state** when `[graph].enabled = true` (`graph_ner_status`, `model_validation.py`), prepended ahead of the probe warnings. It reports either that the `[graph]` extra is missing (`gliner` is not importable) or that the probe could not determine it. It does **not** report the model checkpoint: that is fetched lazily on first ingest, and a failed checkpoint load degrades per-document rather than surfacing here. This one is **not** search-affecting either way: it means prose entity/relation extraction is degraded, not that a query will fail. Fix it with `pip install 'archon-search[graph]'` or by re-running `archon-search wizard` — see [`60_graph_operations.md`](60_graph_operations.md#provisioning-the-prose-extraction-model); triage it separately from a GPU/provider warning even though both raise `checks.models` to `warn`.

Every entry in `provider_warnings` is operator-actionable **by construction** — `checks.models` grades that field alone, so anything permanent and unactionable put there would pin the check to `warn` for the life of the deployment. There is deliberately no sibling channel for an unactionable disclosure; the `provider_notes` field that once carried one has been removed and nothing replaced it.
...
| Critical | `GET /ready` 503 with `checks.storage == "fail"` | 2 consecutive failures, 60s apart | Storage connection down — search is broken even though the process is live. |
| Warning | `GET /ready` 503 with `checks.storage == "ok"` (i.e. `checks.models` and/or `checks.sync == "pending"`) | Still 503 more than 10 minutes after process start | Eager model warm-up or the startup collection sync is abnormally slow. Some pending time is normal right after every restart — a large corpus can legitimately keep the startup sync pending past a Docker `start-period` — but a sustained pending state past this threshold points at a stuck model load or an oversized/stuck sync. |
| Warning | `GET /ready` (any status) with `checks.models == "fail"` | Any | Eager warm-up failed terminally (task raised, or exceeded `_EAGER_WARMUP_TIMEOUT_SECONDS` = 600s). **Not Critical**: it does not gate `ready`, and the server falls back to lazy loading, so search still works — check the log for `"eager model warm-up failed: ..."` and investigate why the load failed (download error, disk space, invalid `[database].embedding_model`). |
