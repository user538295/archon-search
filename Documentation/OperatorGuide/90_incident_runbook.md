...
- **WARN** → both models loaded, but `provider_warnings` is non-empty. Read the list — it has exactly two sources, and they need different responses. (a) An **ONNX provider or probe warning** (`"configured ONNX providers not available: ..."`, `"embedder probe failed: ..."`, split-provider validation): check `[database].providers` against what the installed onnxruntime build supports. (b) A **graph NER warning** (2026-08-19-030) when `[graph].enabled = true` — either the `en_core_web_sm` model is missing, or it is present but the deployment is `multilingual = true` so prose extraction is English-only. The graph cause does **not** affect search; it degrades prose entity extraction at ingest. Fix with `archon-search wizard`, or place the model by hand — [60_graph_operations.md](60_graph_operations.md#provisioning-the-spacy-ner-model). HyDE / RAG-Fusion provider reachability never appears here; read the `hyde` / `rag_fusion` sub-objects on `/status` instead.
