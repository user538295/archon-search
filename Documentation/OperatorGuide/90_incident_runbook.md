...
- **WARN** → both models loaded, but `provider_warnings` is non-empty. Read the list — it has three sources, and they need different responses. (a) An **ONNX provider or probe warning** (`"configured ONNX providers not available: ..."`, `"embedder probe failed: ..."`, split-provider validation): check `[database].providers` against what the installed onnxruntime build supports. (b) A **graph NER warning** (2026-08-19-030) when `[graph].enabled = true` — the `en_core_web_sm` model is either missing, or present under the data directory but incompatible with the installed spaCy (the remedies differ: provision vs. re-provision, and the warning says which). (c) `"graph NER model presence could not be determined"` — the probe itself failed; check the data directory is readable. The English-only disclosure is **not** in this list: it lives in `provider_notes`, which `checks.models` does not grade. The graph cause does **not** affect search; it degrades prose entity extraction at ingest. Fix with `archon-search wizard`, or place the model by hand — [60_graph_operations.md](60_graph_operations.md#provisioning-the-spacy-ner-model). HyDE / RAG-Fusion provider reachability never appears here; read the `hyde` / `rag_fusion` sub-objects on `/status` instead.
...
- **Graph has code symbols but no prose entities; ingests succeed with a warning** → the `en_core_web_sm` NER model is missing. Ingest degrades rather than failing (2026-08-19-030): chunks embed and persist, code-symbol nodes are written, prose NER is skipped, and `IngestResult.warnings` carries `"spaCy model 'en_core_web_sm' is unavailable; prose entity extraction is disabled for this ingest ..."`. The server logs the matching WARNING **once per process** — a single line, not one per file — so grep the log from process start, and confirm from outside with `GET /status` → `model_validation.provider_warnings`. Remedy: `archon-search wizard`, or a manual placement under `<data-dir>/models/spacy/` ([60_graph_operations.md](60_graph_operations.md#provisioning-the-spacy-ner-model)). Re-ingest afterwards — degraded documents are not retroactively extracted. `python -m spacy download en_core_web_sm` does **not** work in a `uv tool install` deployment (no package installer in that venv).
