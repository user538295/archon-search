...
- `eager_load_embedders` (false). When true, ONNX weights are reconstructed at startup — for the embedder cache *and* the reranker cross-encoder — removing the first-query latency spike. Warm-up runs as a background task, so it does not delay the port binding; queries that arrive before it finishes still pay the lazy load. Because startup no longer waits for it, poll `GET /ready` for completion — it answers `503` with `checks.models: "pending"` until the task finishes (or a terminal `_EAGER_WARMUP_TIMEOUT_SECONDS` ceiling of 600s is hit, which forces `warmup_result = "failed"`), and `200` after — with `checks.models: "fail"` (not `"ok"`) if that terminal state was `"failed"`, since a wedged/crashed warm-up must not gate readiness forever any more than it should silently read as healthy — assuming the lifespan's independent startup collection sync has also finished by then (see `GET /ready` in `160_operational_readiness_monitoring_and_reliability.md`). The log carries the same signal: `eager model warm-up complete (N embedding models + reranker)` at INFO, `eager model warm-up failed: ...` at WARNING (a finished-or-failed warm-up clears its own `503` cause — it is not a permanent 503, but `ready` can still be `false` for the unrelated startup-sync reason). A failed warm-up is non-fatal — the server keeps serving and falls back to lazy loading.
