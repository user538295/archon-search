...
- `eager_load_embedders` (false). When true, ONNX weights are reconstructed at startup — for the embedder cache *and* the reranker cross-encoder — removing the first-query latency spike. Warm-up runs as a background task, so it does not delay the port binding; queries that arrive before it finishes still pay the lazy load. Because startup no longer waits for it, poll `GET /ready` for completion — it answers `503` with `checks.models: "pending"` until the task finishes, and `200` after. The log carries the same signal: `eager model warm-up complete (N embedding models + reranker)` at INFO, `eager model warm-up failed: ...` at WARNING (a failed warm-up sets `ready` back to true — it is not a permanent 503). A failed warm-up is non-fatal — the server keeps serving and falls back to lazy loading.
