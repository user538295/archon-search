...
- **Test-suite parallel execution (C10 + C11 + C12):** `pytest-xdist` with `--dist=loadgroup`, session-scoped `connected_store`, `xdist_group("mcp")` on 16 MCP-stub files, `xdist_group("install")` on the three install-lock files, and per-worker native-thread caps in `tests/conftest.py`. CI uses `-n0` explicitly for serial `--cov-append` correctness. See `Architecture/200_testing_strategy.md`.
- **FTS-rebuild bypass in tests (C13):** `SearchPipeline.ingest_directory()` gains `rebuild_fts: bool = True` (keyword-only, default `True`, no production behavior change). All eligible ingest tests that never query FTS now pass `rebuild_fts=False`, eliminating concurrent Tantivy index-build I/O contention under xdist. Default-suite median wall time on a 14-core machine is **~71s** (range 69–74s) — down from ~127s before C13 and from 6.5 min before C10. Implemented in `archon_search/pipeline.py`; tests in `tests/test_pipeline_ingest_fts.py`.
- **Install-lock parallel-test isolation + Path.home() ratchet (C17):** Per-worker `ARCHON_SEARCH_DATA_DIR` autouse fixture eliminates xdist install-lock collisions; `test_no_hardcoded_path_home.py` ratchet (hash-pinned allowlist + AST marker-scope guard) prevents new `Path.home()` callsites outside `paths.py`; `install.py` lines 48/377/1508 migrated to `get_data_dir()`. See `Architecture/200_testing_strategy.md`.
...
- **C9 — Container support (Docker + GHCR).** *Shipped.* `docker run` and `docker compose up` start `archon-search` configured purely via env vars, with all runtime state on a single mounted volume and logs to stderr. CPU (`:latest`) and NVIDIA GPU (`:gpu`) image variants are published to GHCR on tag push via `archon-search-release.yml`. Plan: `Completed/C9-container-support-plan.md`; brief: `Completed/C9-container-support-brief.md`.
- **C13 — Bypass FTS-index rebuild in tests that don't query FTS.** *Shipped.* Added `rebuild_fts: bool = True` to `SearchPipeline.ingest_directory()`; all eligible ingest tests pass `rebuild_fts=False`. Median wall time improved from ~127s to ~71s. See Status Snapshot.
