...
- **C13 — Bypass FTS-index rebuild in tests that don't query FTS.** *Proposed (not scheduled).* Adds `rebuild_fts: bool = True` to `SearchPipeline.ingest_directory()` (mirroring `ingest_file`); estimated impact 127s → 90–100s default-suite median wall time. Brief: `Backlog/C13-fts-rebuild-test-bypass-brief.md`.
