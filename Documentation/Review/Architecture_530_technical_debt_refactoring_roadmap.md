...
- **CON-5** — `routes_search.py:82–84` catches `Exception` from `pipeline.search`, logs at `warning`, and returns `SearchResponse(results=[], acl_filtered=False)` with implicit 200. Meta failure → 503 (`routes_search.py:71`). All line citations match.
