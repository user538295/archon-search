#!/usr/bin/env bash
# Publishes or updates all archon-search GitHub release notes to AAA quality.
# Requires: GITHUB_TOKEN with contents:write permission on user538295/archon-search
# Usage: bash scripts/publish_release_notes.sh

set -euo pipefail

REPO="user538295/archon-search"
...
if [[ "$STATUS" == "403" ]]; then
  echo "ERROR: GITHUB_TOKEN lacks contents:write permission." >&2
  echo "Fix: go to https://github.com/settings/tokens, edit the fine-grained PAT," >&2
  echo "and set Repository permissions > Contents = Read and write." >&2
  exit 1
fi

echo "Token OK (status $STATUS). Publishing release notes..."

# ──────────────────────────────────────────────────────────────────────────────
# Helper: create or update a release
# ──────────────────────────────────────────────────────────────────────────────
upsert_release() {
  local tag="$1"
  local name="$2"
  local body="$3"
  local existing_id="$4"   # pass "" to create
...
" "$tag" "$name" "$body")
  ...
  if [[ -n "$existing_id" ]]; then
    echo "  Updating $tag (id=$existing_id)..."
    curl -s -X PATCH \
      -H "Authorization: Bearer $GITHUB_TOKEN" \
      -H "Content-Type: application/json" \
      "${API}/${existing_id}" \
      -d "$payload" | python3 -c "import sys,json; r=json.load(sys.stdin); print('  -> OK:', r.get('html_url','?')) if 'html_url' in r else print('  -> ERROR:', r)"
  else
    echo "  Creating $tag..."
    curl -s -X POST \
      -H "Authorization: Bearer $GITHUB_TOKEN" \
...
# ──────────────────────────────────────────────────────────────────────────────
# 26.5.0 — Initial standalone release
# ──────────────────────────────────────────────────────────────────────────────
BODY_260="**Initial standalone release: full hybrid retrieval server extracted from the Archon monorepo**
...
- **Opt-in telemetry:** JSONL per-call logging to \`~/.archon-search/search-logs/\`; no raw query strings logged (structural invariant); \`/telemetry/stats\` and \`/telemetry/entries\` endpoints; pruner with configurable \`retention_days\`
...
upsert_release "26.5.0" "26.5.0" "$BODY_260" ""

# ──────────────────────────────────────────────────────────────────────────────
# 26.5.333 — Automated CI/CD
# ──────────────────────────────────────────────────────────────────────────────
BODY_333="**Automated CI/CD: PyPI OIDC publish, PR gate, and eval gate**
...
upsert_release "26.5.333" "26.5.333" "$BODY_333" ""

# ──────────────────────────────────────────────────────────────────────────────
# 26.5.645 — A–C series: hardening, observability, routing, live eval, install
# ──────────────────────────────────────────────────────────────────────────────
BODY_645="**A–C series: search hardening, observability, multi-collection routing, live eval, structured logging, tiered install, git-cliff CHANGELOG**
...
- A5: \`validate_ingest_path\` + \`PathUnsafeError\` wired into all ingest endpoints and MCP tools; \`_where_eq\`/\`_where_in\` SQL fragment helpers replace all f-string SQL in \`store.py\`; CI guard (\`test_no_fstring_sql.py\`) fails the build if f-string \`.where()\`/\`.delete()\`/\`.count_rows(\` reappears; \`StoreBusyError\` → 503 on ingest endpoints; MCP surfaces it as \`code=store_busy\`
...
- A7: durable-write helper with crash-injection tests; 5 persistent state files (collection meta, indexing state, ACL, telemetry, key file) migrated to fsync-safe write pattern; telemetry persistent-fd with rotate-only fsync; \`OSError\` → 500 routing; CI lint gate blocks regression
...
upsert_release "26.5.645" "26.5.645" "$BODY_645" ""

# ──────────────────────────────────────────────────────────────────────────────
# 26.5.647 — Fix: fractional seconds in captured_at regex
# ──────────────────────────────────────────────────────────────────────────────
BODY_647="**Fix: fractional seconds rejected in captured_at timestamp regex**
...
upsert_release "26.5.647" "26.5.647" "$BODY_647" "332132881"

# ──────────────────────────────────────────────────────────────────────────────
# 26.5.652 — Release automation: backfill workflow + flat note format
# ──────────────────────────────────────────────────────────────────────────────
BODY_652="**Release automation: backfill workflow + flat release note format**

- Added \`workflow_dispatch\` GitHub Actions workflow (\`archon-search-backfill-release-notes.yml\`) to retroactively create or update release notes for all existing tags in one run
...
upsert_release "26.5.652" "26.5.652" "$BODY_652" ""

# ──────────────────────────────────────────────────────────────────────────────
# 26.5.654 — Fix: dynamic git-cliff download URL in CI
# ──────────────────────────────────────────────────────────────────────────────
BODY_654="**Fix: dynamic git-cliff download URL in CI**
...
upsert_release "26.5.654" "26.5.654" "$BODY_654" "332198797"
...
for r in sorted(releases, key=lambda x: x['tag_name']):
    ...
    print(f'  {r[\"tag_name\"]:12} id={r[\"id\"]}  body_chars={body_len}  url={r[\"html_url\"]}')
"

echo ""
echo "Done. Visit https://github.com/user538295/archon-search/releases to verify."
