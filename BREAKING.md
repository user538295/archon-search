...
**Surface**: `POST /ingest` (and directory/MCP ingest); `GET /status` → `model_validation.provider_warnings` and the new `model_validation.provider_notes`; `GET /ready` → `checks.models`.

**Behaviour change** (previously-failing ingests now succeed — non-breaking for well-behaved clients; one additive response field):
...
- `provider_warnings` (`GET /status` → `model_validation`) can newly carry graph-NER entries when `[graph].enabled = true`: a missing or version-incompatible model. On upgrade, any existing `[graph].enabled = true` deployment that lacks the model gains one of these entries.

- `model_validation` gains an **additive** `provider_notes: list[str]` field on `GET /status`. It carries the English-only disclosure for `[database].multilingual = true` deployments — deliberately NOT in `provider_warnings`, because `checks.models` grades that field and a permanent, unactionable note there would pin `GET /ready` to `"warn"` for the life of the deployment. Clients that ignore unknown fields are unaffected.
- A deployment that lacks the model can flip `checks.models` (`GET /ready`) from `"ok"` to `"warn"` fleet-wide, purely from the upgrade — no config change on the operator's part. `provider_warnings` was already `list[str]` and `CheckStatus.WARN` already existed for `checks.models`, so this is not a schema break, but `Documentation/OperatorGuide/20_monitoring_and_alerts.md` documents alerting on exactly this field — operators with that alert wired up should expect new, non-search-affecting `"warn"` entries after upgrading and triage them separately from a GPU/provider misconfiguration (see that doc for the distinction).
